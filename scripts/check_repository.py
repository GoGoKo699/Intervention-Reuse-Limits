...
def main() -> None:
    ...
    for name in ('checkpoint','finite_accuracy','quadrature','minimal_realization','response_lower_bounds'):
        report = json.loads((ROOT/f'reports/{name}.json').read_text())
        require(report['status']=='PASS',f'Saved report is not PASS: {name}')
    for report_name in ('finite_accuracy','quadrature','minimal_realization','response_lower_bounds'):
