...
def main() -> None:
    ...
    for name in ('checkpoint','finite_accuracy','quadrature','minimal_realization','response_lower_bounds','unrestricted_rate_lower_bound','finite_field','path_information','actuator_hierarchy','general_compression','reversible_compression','general_controlled_lower_bound'):
        report = json.loads((ROOT/f'reports/{name}.json').read_text())
        require(report['status']=='PASS',f'Saved report is not PASS: {name}')
    for report_name in ('finite_accuracy','quadrature','minimal_realization','response_lower_bounds','unrestricted_rate_lower_bound','finite_field','path_information','actuator_hierarchy','general_compression','reversible_compression','general_controlled_lower_bound'):
