...
The current lead is the [four-endpoint covariance witness](FAMILIAR_SWITCH_PREPARATION_WITNESS.md). It adds a second experimental resource: both low- and high-field equilibrium preparations must be available. One shared predictor explains the preparation/word pairs $(0;H),(H;0),(H;0H),(0;H0)$. A quadratic expression in their four means measures conditional covariance between two response propensities inside either observed conformation. A singleton sector has zero covariance; the two-switch target has positive covariance in both. Arbitrary ordinary rivals with at most three states therefore fail one of these tests, even with unbounded rates or non-embeddable reversible tick kernels.
...
An [exact-only snapshot corollary](FAMILIAR_SWITCH_PREPARATION_WITNESS.md) replaces high-field preparation by a noninvasive initial binary readout. Both opposite-order protocols $0H$ and $H0$ start from $\pi_0$ and record their initial/final joint laws. Their maximum TV error is $>1/1000$ for every ordinary rival with at most three states; the same unrestricted three-state predictor matches both laws exactly. Thus joint-TV tolerances $0\le\delta_{\rm joint}\le1/1000$ give minima three and four. This uses exact equilibrium preparation and Gibbs tilt, with a changed observation resource. It does not inherit the calibrated endpoint sample counts or imply full multitime path-law equality.
...
The four-cell comparison reduces the sufficient endpoint budget by changing preparation access; it does not prove a corresponding improvement in the optimal test under unchanged resources. Its deterministic gap is already nearly fixed by the certified comparator. The next questions are supportable preparation, statistical efficiency and a larger effect under explicitly counted protocols or readouts. The [earlier margin source comparison](FAMILIAR_SWITCH_MARGIN_SOURCE_AUDIT.md) and [kinetic source audit](FAMILIAR_SWITCH_SOURCE_AUDIT.md) retain their attribution boundaries. No microscopic device realization, generalized-reversal separation or universal dissipation necessity is claimed.
...
The next priorities are a larger simple observable separation or more efficient experimental test, a microscopic justification of the stipulated force-controlled model, and tighter identity-reversal entropy frontiers. The calibrated finite state gap and explicit sample bounds do not establish experimental feasibility; a physical realization of the word involution is also unresolved. The stronger uncapped law, a tight-band binary construction and matching quantitative bounds remain open. Manuscript drafting remains the final step.
