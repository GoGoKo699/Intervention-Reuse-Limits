...
For a target with each bit-error probability at most one percent, the equal-rate fixture has nominal recorded joint-TV gap $>0.000375$ against this larger rival class. With the same physical allowances, the gap is $>0.0002$ and the constructive three-state error is at most $2\times10^{-5}$, giving actual minima three and four on $[2\times10^{-5},0.0002]$. The [new verifier](../scripts/verify_switch_physical_interface.py) and [report](../reports/switch_physical_interface.json) supply the finite certificates. The separate localized confidence-box test uses **508 million independent paired trials** in total, equally divided between the orders, or **1.016 billion binary readouts**, for false rejection below 5% and power above 95%. It tests the exact composite null with its physical allowances and no additional approximation allowance. This conservative sufficient budget is not optimal, and the calibrated 1.2-million-trial design above does not transfer to the larger unknown-detector null.
...
The current PRL-directed exploration centers on the two-switch covariance mechanism, its unknown-symmetric-detector extension and a conditional charge-model realization. The short experiment and exact state separation are clear, while the detector-unknown sufficient sampling budget and initial-instrument assumptions remain substantial obstacles. The earlier asymptotic results separately distinguish ordinary detailed balance from generalized equilibrium: a polynomial predictor with a reversal involution has zero generalized stationary entropy production, so no implementation-independent dissipation requirement follows. The assessment remains continued exploration; manuscript drafting remains on hold.

The current conclusions distinguish prediction tasks and predictor constraints.

| Question | Result | Essential limitation |
|---|---|---|
| Can the two-order witness tolerate unknown detector error probabilities? | [The scalar contrast-elimination witness](FAMILIAR_SWITCH_UNCALIBRATED_READOUT.md) excludes all ordinary rivals with at most three states and arbitrary fixed symmetric bit errors up to one half; the stated physical target has recorded gap $>0.0002$. | The channel form, preparation and joint instrument remain specified. The separate conservative 508-million-paired-trial test is not the calibrated score test and is not claimed optimal. |
