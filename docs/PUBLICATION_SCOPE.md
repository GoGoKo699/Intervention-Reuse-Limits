...
The score tests improve sufficient budgets within each specified observation design; comparing endpoint and snapshot totals also changes preparation and readout resources. The deterministic gaps are tightly bracketed by fixed feasible ordinary comparators. Further work should address supportable preparation and initial measurement, statistical efficiency and larger effects under explicitly counted protocols or observations. The [earlier margin source comparison](FAMILIAR_SWITCH_MARGIN_SOURCE_AUDIT.md) and [kinetic source audit](FAMILIAR_SWITCH_SOURCE_AUDIT.md) retain their attribution boundaries. No microscopic device realization, generalized-reversal separation or universal dissipation necessity is claimed.
