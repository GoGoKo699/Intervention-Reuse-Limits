...
Eleven specified endpoint-mean experiments, each using at most five ticks and three field segments, distinguish these counts at every fixed positive clock. No rival rate cap is needed, even for existence of a positive error margin on that same menu. That margin is not yet quantified. The three-state construction works for arbitrary nonnegative-field protocols, with exits below two, but establishes mean equality rather than binary path-law equality. The passive visible path already has memory; this is a separate target and interface from the earlier hub models.

The [protocol study](FAMILIAR_SWITCH_PROTOCOLS.md) keeps the precision question open: twelve two-switch fixtures each admit a retained ordinary three-state comparator below $0.14\%$ occupation error on a 33-protocol screen. These feasible numerical uppers are neither global optimal fits nor all-protocol bounds, and do not certify a lower error margin. The [source audit](FAMILIAR_SWITCH_SOURCE_AUDIT.md) attributes the familiar kinetics and classical closure, distinguishes the controlled realization theorem, and records unresolved source access. No microscopic device realization, generalized-reversal separation or universal dissipation necessity is claimed.
