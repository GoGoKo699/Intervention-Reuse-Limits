...
The [research dossier](RESEARCH_DOSSIER.md) and [claim ledger](CLAIM_LEDGER.md) supply the model, quantified statements, dependencies, reproducibility and source links for eventual drafting. The latest result is a [superpolynomial reversibility penalty without a rival rate cap](UNCAPPED_REVERSIBILITY_LOWER_BOUND.md). On the nineteen-level family,

$$
D_{\rm all}^{\rm unc}(\delta)=\delta^{-\Theta(1)},\qquad
D_{\rm rev}^{\rm unc}(\delta)\ge\exp\!\left(\exp\!\left(c_H\sqrt{\log(1/\delta)}\right)\right).
$$

Both classes preserve the original field rule, exact histogram, preparation and readout, and may construct their own states. The proof uses thirty-nine fixed fields and arbitrarily rapid switching. The target band remains $[k,3k]$; no rival rate cap or lower gap is imposed. This quantitative result supersedes the earlier qualitative-only uncapped assessment. The stronger lower $\exp(c\delta^{-\alpha})$, an uncapped binary extension and a fixed-clock superpolynomial separation remain open. The uncapped lower and available reversible upper do not match.

The [bounded-control resolvent theorem](BOUNDED_CONTROL_RESOLVENT_OBSERVABILITY.md) supplies the new bridge from actual means to positive words. A separate [Walsh hierarchy](UNCAPPED_WALSH_OBSERVABILITY.md) gives fixed-clock, fixed-target rank thresholds against arbitrary Markov rivals, plus an exact 38-state smallest-target quotient. The [new source comparison](BOUNDED_WORD_RECOVERY_PRIOR_ART.md) attributes classical ingredients and bounds the novelty assessment. Manuscript drafting remains the final step.
...
An orthogonal query Gram matrix separately gives the polynomial lower against arbitrary Markov predictors through the existing target-only rank argument. This fixed-clock entropy proof needs the rival's original field rule, exact histogram and fixed cap. The later uncapped theorem removes the cap using different bounded observations and arbitrarily rapid switching; arbitrary reversible field dependence remains outside both results. The nineteen actuator values are fixed independently of accuracy. The binary theorem above removes that alphabet requirement at the larger budget $6580k$; it does not reduce the present $3k$ construction to two levels.
...
The uncapped theorem strengthens the case for an ambitious theoretical paper: a superpolynomial cost of ordinary reversibility persists with arbitrarily fast hidden dynamics. The binary theorem separately supplies a stronger growth class under a shared rate budget and finite clock resolution. This assessment remains conditional on external proof scrutiny and complete-theorem novelty. The original field rule, exact histogram and differing control-clock assumptions must remain explicit. Manuscript drafting remains on hold.

The current conclusions distinguish prediction tasks and predictor constraints.

| Question | Result | Essential limitation |
|---|---|---|
| Does a reversibility penalty survive arbitrarily fast hidden rates? | [The uncapped nineteen-level theorem](UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) gives polynomial unrestricted cost and reversible cost at least $\exp(\exp(c_H\sqrt{\log(1/\delta)}))$. | Original rule and exact histogram; arbitrarily rapid switching; no matched growth law or fixed-clock superpolynomial claim. |
...
The main candidate publication story is that ordinary reversibility can force superpolynomially more physical states for reusable prediction, even when hidden rates are unrestricted. Bounded resolvent observation, positivity and entropy connect the physical mean norm to a state-count obstruction. The balanced binary theorem offers a complementary stronger growth law under a fixed rate budget. The passive/step/switching comparison, aggregation separation and positive compression routes remain supporting results with their own assumptions.

The next priority is an uncapped superpolynomial separation with a balanced binary actuator. The nineteen-level proof uses distinct observable port projections, while the binary marker is a general positive operator on rival states; a literal palindromic heat/resolvent substitution has the previously proved PSD obstruction. A successful extension needs an observable positive gate construction compatible with the weighted binary repair. A fixed-clock uncapped separation and the stronger $\exp(c\delta^{-\alpha})$ uncapped lower are distinct open questions. The rare-fast-component example still rules out direct uniform raw-moment transfer. Reducing the binary rate budget, arbitrary reversible field dependence, arbitrary fixed expander labels, optimal exponents and original-rule constant-step compression remain separate refinements.
