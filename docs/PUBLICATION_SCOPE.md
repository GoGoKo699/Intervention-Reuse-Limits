...
The main remaining mathematical questions are the optimal bounded-binary exponents and polynomial reversible sufficiency under a spectral cap alone on general graphs. The new expander result resolves a high-probability fixed-label class despite a uniform obstruction to small-cost fragmentation. Removing the labeling assumption is a concrete next boundary. The analytic constant-step construction resolves qualitative field regularity; imposing the original field rule or uniform derivative bounds remains a separate refinement. The general bounded-sensitivity upper bound can also be improved, but it concerns a broader class. No equality of optimal reversible and unrestricted exponents is proved.
