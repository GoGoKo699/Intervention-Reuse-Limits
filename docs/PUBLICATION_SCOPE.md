...
The current physical question is whether an equilibrium system can require stationary irreversible currents in a small model that predicts interventions accurately. Four completed advances support that question:

| Core result | Established advance | Scope |
|---|---|---|
| [Positive label selectors](POSITIVE_LABEL_SELECTORS.md) | Two physical fields recover positive approximate selectors for the target's nineteen actuator levels. | Rival sensitivities may lie anywhere in the same bounded interval; they need not reproduce the target alphabet or histogram. |
| [Tight-band fixed-clock separation](TIGHT_BAND_FIXED_CLOCK_REVERSIBILITY.md) | The original target band $[k,3k]$ admits polynomial unrestricted prediction, while ordinary reversible prediction needs superpolynomially many states. | Any fixed positive clock and two fields suffice, with arbitrary finite rival rates and new states. The original field rule, preparation and readout remain fixed. |
| [Uniform entropy-production reversibilization](ENTROPY_PRODUCTION_REVERSIBILIZATION.md) | Arithmetic reversibilization preserves the states, stationary masses, labels and every exit rate, with an all-protocol, all-horizon endpoint-mean bound. | Its constant depends on the external interface, without a hidden rate, dimension or minimum-mass factor. |
| [State–entropy-production frontier](STATE_ENTROPY_PRODUCTION_TRADEOFF.md) | Reversible state lower bounds extend to predictors with bounded stationary entropy production; polynomial predictors also admit a finite-entropy-production construction. | The broad uncapped frontier and the stronger common-cap, exact-histogram frontier have different assumptions; neither frontier is matched. |

For every fixed positive clock $a/k$, the original nineteen-level family satisfies
...
Both classes use the original exponential kinetic field rule, stationary zero-field preparation and binary readout, and may construct their own states. Rivals may have arbitrary sensitivities in $[-G,G]$, $G=9/100$, with no histogram matching, rate cap or lower gap. Only the two fields $0,H$ and clock-endpoint means are needed. The target is unchanged: no diagnostic states or rate rescaling are added, and its hidden relaxation band remains $[k,3k]$. Target index $n$ has a finite witnessing horizon of at most $C_{a,H}(n+1)^5$ ticks at error $e^{-C_{a,H}(n+1)^5}$. This does not bound experiment count, sample cost or required parameter precision.

The two-field statement gives a polynomial unrestricted **upper**. The thirty-nine-field corollary gives matching polynomial necessity on the same original target family. The [Gram recovery](FIXED_CLOCK_GRAM_OBSERVABILITY.md), positive selectors and [weighted repair](WEIGHTED_WHOLE_WORD_REPAIR.md) form the new proof chain. The [internal review](PRL_EXPLORATION_INTERNAL_REVIEW.md), [bounded verifier](../scripts/verify_prl_exploration.py), [report](../reports/prl_exploration.json) and [primary-source comparison](PRL_EXPLORATION_SOURCE_AUDIT.md) have separate evidentiary roles; they do not constitute external validation.

For $R=e^{(1+G)H}$ and full zero-field stationary entropy-production rate $\sigma_0$, the new endpoint comparison is

$$
\mathcal D_H(F,F_{\rm sym})\le R^{3/2}\sqrt{\sigma_0/k},\qquad
D_{\le\Sigma}(\delta)\ge D_{\rm rev}(\varepsilon),\quad
\varepsilon=\delta+R^{3/2}\sqrt{\Sigma/k}.
$$

The broad rival class therefore inherits the fifth-root-log state lower at small $\varepsilon$. Separately, under the older common exit cap $3k$ and exact nineteen-level histogram, the stronger lower is $D_{\le\Sigma}(\delta)\ge\exp(c\varepsilon^{-\alpha})$. A uniform polynomial state budget in that narrower class requires worst-case $\Sigma\ge c'k[\log(1/\delta)]^{-2/\alpha}$. A polynomial predictor with $\sigma_0\le(3k/2)\log(12R/\delta)$ exists in both classes. These bounds concern stationary path irreversibility under ordinary time reversal; they do not identify state count with stationary Shannon entropy, or stationary entropy production with driven heat or device power.

Earlier comparisons remain useful and retain their own assumptions. The [binary fixed-clock uncapped theorem](FIXED_CLOCK_UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) uses a rescaled tagged target with a large rate-to-gap ratio and inner exponent $1/12$. The earlier [binary](BINARY_UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) and [nineteen-level](UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) uncapped theorems give inner exponent $1/2$ with arbitrarily rapid switching and their specified histogram constraints. The older fixed-budget theorems supply singly exponential inverse-error lower bounds at every fixed positive clock. These guarantees are not merged.

The next priorities are physical generality and assumption auditing, a transparent certified finite-tolerance witness, and tighter entropy-production frontiers. Improving the fifth-root bookkeeping exponent is secondary to those questions. The stronger uncapped law $\exp(c\delta^{-\alpha})$, a tight-band binary construction and matching resource bounds remain open.
...
An orthogonal query Gram matrix separately gives the polynomial lower against arbitrary Markov predictors through the existing target-only rank argument. This older fixed-clock entropy proof needs the rival's original field rule, exact histogram and fixed cap. The [new tight-band fixed-clock theorem](TIGHT_BAND_FIXED_CLOCK_REVERSIBILITY.md) removes the rival cap and histogram requirement on this same target family, with a weaker quantitative lower. Arbitrary reversible field dependence remains outside these results. The nineteen actuator values are fixed independently of accuracy. The binary theorem above removes that alphabet requirement at the larger budget $6580k$; it does not reduce the present $3k$ construction to two levels.
...
The PRL-directed exploration now has a quantitative physical resource question: how much stationary irreversibility can a reduced stochastic model avoid while remaining small and accurate under control? The original tight-band target, a fixed positive control clock and unrestricted rival rates now coexist in one separation theorem. The endpoint reversibilization bound connects that obstruction to stationary entropy production without a hidden-activity factor. The original kinetic interface and ordinary time-reversal convention remain substantive assumptions. The assessment is to continue exploration while testing physical generality, finite-tolerance relevance and the closest complete-theorem comparisons; manuscript drafting remains on hold.

The current conclusions distinguish prediction tasks and predictor constraints.

| Question | Result | Essential limitation |
|---|---|---|
| Can a tight-band equilibrium target force a reversibility penalty at a fixed clock without a rival rate cap? | [The original nineteen-level family](TIGHT_BAND_FIXED_CLOCK_REVERSIBILITY.md) has a polynomial unrestricted upper and reversible lower $\exp(\exp(c_{a,H}[\log(1/\delta)]^{1/5}))$ using only $0,H$. | Original field rule, preparation and readout remain fixed; rival labels may be arbitrary in $[-G,G]$ with no histogram matching. Matching unrestricted polynomial necessity uses the thirty-nine-field menu. |
| Does small stationary entropy production imply a nearby reversible predictor on the same states? | [Arithmetic reversibilization](ENTROPY_PRODUCTION_REVERSIBILIZATION.md) gives all-protocol, all-horizon endpoint-mean error at most $R^{3/2}\sqrt{\sigma_0/k}$. | This concerns endpoint responses and ordinary time reversal; it does not bound complete long-path error or driven heat. |
| Can a state budget force stationary irreversibility? | [The resource frontier](STATE_ENTROPY_PRODUCTION_TRADEOFF.md) gives $D_{\le\Sigma}(\delta)\ge D_{\rm rev}(\delta+R^{3/2}\sqrt{\Sigma/k})$ and a polynomial-state finite-entropy-production upper. | The broad uncapped frontier is weaker than the older common-cap, exact-histogram frontier; neither is matched and no positive limiting entropy-production floor is proved. |
...
The leading publication question is whether accurate reusable prediction of an equilibrium system imposes an unavoidable state–irreversibility tradeoff. The new result retains the original target band $[k,3k]$ and survives a fixed clock, arbitrary rival rates and arbitrary bounded rival actuator labels. Its positive observation calculus and entropy bridge connect the actual controlled-mean norm to counted states and stationary probability currents. The stronger capped theorem supplies a separate quantitative frontier; the passive/step/switching comparison and positive compression routes remain supporting results with their own assumptions.

The next work should test which field rules and preparations preserve the obstruction and the reset-based entropy bound, audit the physical meaning of the ordinary reversal convention, and seek a small certified witness at a useful tolerance. The present asymptotic construction and polynomial witnessing horizon do not supply practical sampling guarantees. A small target's rank threshold alone does not establish a reversible-versus-irreversible state advantage. On the resource side, narrowing the inverse-polylogarithmic necessary and logarithmic sufficient entropy-production budgets under the common cap would be more consequential than sharpening a proof-bookkeeping exponent. The stronger uncapped state law, a tight-band binary realization and matched frontiers remain separate open questions.

The constructor receives a known target. Observation results concern specified-model tests with the stated preparation and readout. Continue with bounded analytic work and small exact checks; a large simulation or experimental platform is not required to resolve the current proof questions. PRL is the user's selected target, while complete-theorem novelty and the journal's significance threshold remain matters for further assessment. Manuscript writing remains the final step.
