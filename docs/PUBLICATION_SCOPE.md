...
Five new proof notes define the current scope:
...
The next work should establish a recognizable physical model class in which the relevant parity and interface assumptions are justified, or supply a small certified ordinary-versus-generalized state advantage at a useful tolerance. The current address-and-table target and asymptotic clock witnesses do not provide that finite example. A target's rank threshold alone does not establish a reversible-versus-irreversible state gap. Narrowing the inverse-polylogarithmic necessary and logarithmic sufficient identity-reversal entropy budgets remains useful when identity reversal is physically appropriate. A stronger uncapped ordinary lower, a tight-band binary realization and matching frontiers remain separate open questions.
