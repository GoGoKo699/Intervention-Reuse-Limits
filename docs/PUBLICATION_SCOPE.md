...
**Expanded research assessment, 22 September 2026.** The main candidate is now an exponential-versus-polynomial cost of ordinary reversibility for actual controlled-mean prediction. The [dynamic-lamp theorem](DYNAMIC_LAMP_REVERSIBILITY_LOWER_BOUND.md) compares predictors of the same target family, with the same original kinetic field rule, exact nineteen-level actuator histogram, stationary zero-field preparation, binary readout, and internal outgoing-rate budget $3k$. Stationary nonreversible predictors have polynomial worst-case state growth in $1/\delta$; reversible predictors have singly exponential growth in a positive power of $1/\delta$. The latter lower allows arbitrary new states. It is not restricted to aggregation or inherited target coordinates.

The targets and reversible upper models have internal relaxation band $[k,3k]$. The lower covers every original-rule reversible rival with any fixed internal spectral cap, and using cap $6k$ includes all such rivals with exits at most $3k$. Both lower bounds already hold for thirty-nine fixed fields at any fixed positive control clock, with witnessing horizons logarithmic in inverse error. The upper bounds serve all bounded protocols and all horizons. Exponents are not matched. Binary actuators, removal of the rival rate cap, and arbitrary reversible field rules remain outside the new separation.

The candidate proof contribution is the quantitative passage from actual means to positive stationary couplings on a rival's own states. Field interpolation exposes transitions that query and flip independently addressed table bits. Reversibility supplies reverse moments; a fixed cap transfers mean accuracy to those moments; balanced transport repair produces stationary couplings without adding states. Approximate invariance under every decoded bit flip forces high entropy and exponentially many states. Lamp growth, entropy inequalities, logarithm expansions, and positive transport methods are established ingredients. The [source comparison](PRIOR_ART.md) distinguishes them from the combined prediction theorem; the bounded search does not certify originality. Manuscript writing remains on hold.
...
### An exponential cost of ordinary reversibility under a common rate budget
...
An orthogonal query Gram matrix separately gives the polynomial lower against arbitrary Markov predictors through the existing target-only rank argument. The entropy proof, in contrast, needs the rival's original field rule, exact histogram and fixed cap. It does not imply an exponential lower for unrestricted-rate reversible predictors or an arbitrary reversible dependence on the field. The nineteen actuator values are fixed independently of accuracy; a binary version remains the main next target.
...
|---|---|---|
| Can reversibility itself require exponentially more states? | [The dynamic-lamp family](DYNAMIC_LAMP_REVERSIBILITY_LOWER_BOUND.md) has polynomial nonreversible and singly exponential reversible growth under the same internal exit budget $3k$. | Nineteen fixed actuator values, the original field rule, exact histogram and a fixed rival rate cap are essential to the proved scope; exponents are unmatched. |
...
| Can a bounded finite-alphabet predictor preserve reversibility and the spectral band? | Yes, with sufficient count $\exp[C\delta^{-p}\log(2/\delta)]$, retaining the exact actuator histogram. | The dynamic-lamp lower matches the exponential growth class for a fixed nineteen-level family; the binary question remains open. |
...
The main candidate publication story is that ordinary reversibility can force exponentially more physical states for reusable prediction under a common kinetic rule and rate budget. The nineteen-level dynamic-lamp theorem supplies this comparison for arbitrary new-state reversible rivals. The earlier complementary story is that even a common model of every constant-field step can be much smaller than a model reusable under switching. This occurs in the one-dimensional local-walk class with global refresh: passive cost two, logarithmic step cost, and polynomial switching cost, with the last obstruction surviving fixed clock resolution. Polynomial necessary and reversible sufficient switching growth now also holds with one fixed labeling and connected uniformly mixing sparse local expander dynamics. Local tree statistics supply a third positive route beyond density moments and small-cost fragmentation. These statements retain unmatched exponents and precise structural assumptions, with classical random scenery, quadrature, Jacobi, convex selection, concentration, graph, logarithm-series and rank ingredients explicitly acknowledged.

The first next mathematical question is whether the intrinsic separation can be proved with only two actuator values. The present interpolation uses separate port sensitivities, so changing the alphabet requires a new way to expose the relevant positive gate blocks from physical means. Removing the cap on reversible rivals is another major boundary: the present logarithm transfer depends on that cap, and its failure does not prove a cap-free lower. Allowing arbitrary reversible field rules is a further distinct competitor class. The existing binary positive routes, arbitrary fixed expander labels and unmatched exponents remain useful subquestions. The analytic constant-step theorem's original-rule and uniform-derivative refinements concern a separate prediction menu.
