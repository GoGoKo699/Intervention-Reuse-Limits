...
The [research dossier](RESEARCH_DOSSIER.md) and [claim ledger](CLAIM_LEDGER.md) now supply the model, quantified statements, dependencies, reproducibility and source links for eventual drafting. The 23 September continuation adds an explicit nineteen-level [state–speed tradeoff](STATE_SPEED_ACCURACY_TRADEOFF.md), [same-state rate regularization](RATE_REGULARIZATION.md), and [finite-state fast-rate closure](FINITE_STATE_FAST_RATE_CLOSURE.md). The closure yields a positive uncapped error floor for each fixed nineteen-level target and hidden-state budget below $2^{2^n}$, also at a fixed positive clock. It supplies no quantitative accuracy exponent. The exponential inverse-error law without a rival rate cap remains open.
...
The constant $6580$ is a sufficient gadget budget, not an optimal threshold. A binary separation retaining the earlier tight numerical band $[k,3k]$ remains open, as do a quantitative exponential lower for unbounded-rate reversible rivals and arbitrary reversible field rules. The older nineteen-level theorem below supplies a simpler low-budget instance of the same mechanism.
...
The balanced binary theorem makes this a credible candidate for an ambitious theoretical paper: it isolates an exponential state cost of ordinary reversibility in actual controlled prediction under shared physical constraints, beyond the earlier aggregation restriction. This is a research assessment, conditional on independent scrutiny of the proof and complete-theorem novelty; the bounded source audit does not establish priority. The unoptimized rate ratio and the cap and field-rule assumptions must remain visible. Manuscript drafting remains on hold.
...
The main candidate publication story is that ordinary reversibility can force exponentially more physical states for reusable prediction under a common kinetic rule and rate budget. The balanced binary theorem supplies this comparison for arbitrary new-state reversible rivals at a larger fixed budget; the nineteen-level construction retains the tighter $3k$ budget. The earlier complementary story is that even a common model of every constant-field step can be much smaller than a model reusable under switching. This occurs in the one-dimensional local-walk class with global refresh: passive cost two, logarithmic step cost, and polynomial switching cost, with the last obstruction surviving fixed clock resolution. Polynomial necessary and reversible sufficient switching growth now also holds with one fixed labeling and connected uniformly mixing sparse local expander dynamics. Local tree statistics supply a third positive route beyond density moments and small-cost fragmentation. These statements retain unmatched exponents and precise structural assumptions, with classical random scenery, quadrature, Jacobi, convex selection, concentration, graph, logarithm-series and rank ingredients explicitly acknowledged.

The first next mathematical boundary is a quantitative uncapped accuracy law. Fixed-budget compactness and exact actuator purity now give a positive error floor for every fixed nineteen-level target and insufficient fixed state budget, but no rate of decay of that floor. The [rare-fast-component construction](UNBOUNDED_RATE_REVERSIBILITY_BOUNDARY.md#5-rare-fast-states-prevent-raw-moment-transfer) proves that direct uniform recovery of the raw generator moments is impossible; it retains the large target and does not provide compression. A quantitative proof needs a bounded or trimmed observable argument, or a different route. The growing-cap lower and rate-regularization upper currently remain compatible. Reducing the binary gadget budget, particularly to the original target band $[k,3k]$ and exit budget $3k$, is a separate route toward a simpler statement. Allowing arbitrary reversible field rules changes the competitor class further. Arbitrary fixed expander labels, unmatched exponents and the analytic constant-step theorem's original-rule refinement remain distinct questions.
