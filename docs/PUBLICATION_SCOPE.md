...
The constant-step upper now preserves real-analytic field dependence as well as reversibility, irreducibility, stationary zero-field preparation, the exact passive path law and static mean, a uniform full-generator spectral band, and the universal linear and zero quadratic mean responses. It need not obey the target's exponential rate rule, and field derivatives are not uniformly bounded in tolerance. For switching, three sufficient routes now permit polynomial reversible compression inside the original field-rule family: a uniform positive density moment, a polynomial stationary-flux fragmentation profile, or typical fixed binary labels on logarithmic-girth graphs. The third route includes connected uniformly mixing sparse local expanders, where both preceding hypotheses fail uniformly, and its same all-accuracy labeling class contains polynomially hard switching targets. With a spectral cap alone on general graphs, the available reversible upper is singly exponential and polynomial sufficiency remains open. The broader bounded-sensitivity family still has a double-exponential reversible existence bound. The sharp rank-one theorem remains a resolved special case, and the cubic, path-information, and robustness results remain foundations and operational consequences.

The candidate contribution is the polynomial controlled-word obstruction and its realization as a positive reversible physical Markov model with an exactly simple passive law, together with quantitative transfer to actual controlled-mean errors at fixed clock resolution. The new local-walk construction places the full three-task comparison in a one-dimensional local-plus-refresh class, where polynomial switching sufficiency is also achievable reversibly with the original field rule and spectral band. The de Bruijn/Walsh representation, logarithm expansions, interpolation, Markov word approximation, and rank mechanisms are established. The comparisons below distinguish those ingredients from the combined physical prediction theorem; they do not certify originality. Manuscript writing remains on hold during research development.

A separate architectural comparison is now available on one fixed six-level actuator family. Every actuator-respecting stationary-flux partition aggregate can require $\exp(c_a\delta^{-\gamma_a})$ states, while a fresh-state reversible realization has a uniform polynomial sufficient count. Both use the original field rule, exact histogram and advertised internal band. This is a quantitative limitation of that aggregation architecture. The qualitative distinction between aggregation and realization is established in Kotsalis and Shamma's work; the new claim must be assessed for its robust accuracy law and shared physical constraints.
...
The candidate publication story is that even a common model of every constant-field step can be much smaller than a model reusable under switching. This occurs in the one-dimensional local-walk class with global refresh: passive cost two, logarithmic step cost, and polynomial switching cost, with the last obstruction surviving fixed clock resolution. Polynomial necessary and reversible sufficient switching growth now also holds with one fixed labeling and connected uniformly mixing sparse local expander dynamics. Local tree statistics supply a third positive route beyond density moments and small-cost fragmentation. These statements retain unmatched exponents and precise structural assumptions, with classical random scenery, quadrature, Jacobi, convex selection, concentration, graph, logarithm-series and rank ingredients explicitly acknowledged.

The main remaining mathematical questions are the optimal bounded-binary exponents and polynomial reversible sufficiency under a spectral cap alone on general graphs. The architecture theorem now proves that merely improving actuator-respecting stationary-flux partitions cannot give a general polynomial answer: some targets need a different realization despite admitting a polynomial reversible model. The new expander result resolves a high-probability fixed-label class despite a uniform obstruction to small-cost fragmentation. Removing the labeling assumption is a concrete next boundary. The analytic constant-step construction resolves qualitative field regularity; imposing the original field rule or uniform derivative bounds remains a separate refinement. The general bounded-sensitivity upper bound can also be improved, but it concerns a broader class. No equality of optimal reversible and unrestricted exponents is proved.
