...
**Expanded research assessment, 22 September 2026.** The central candidate is a quantitative distinction between passive simplicity, low-order response, and full controlled prediction. For binary sensitivity and a fixed internal rate band, the worst-case physical state count has polynomial growth in inverse actual-mean error: $D_*(\delta)=\delta^{-\Theta(1)}$. The passive process has exactly two states, while a polynomial controlled cost is necessary even with any fixed positive control clock. The lower and upper exponents are not matched.

Reversible predictors with a fixed finite actuator alphabet and rate cap have a singly exponential sufficient count, preserving the complete actuator distribution and spectral band; a polynomial reversible upper bound remains open. The broader bounded-sensitivity family still has a double-exponential reversible existence bound. The sharp rank-one theorem remains a resolved special case, and the cubic, path-information, and robustness results remain foundations and operational consequences.
...
The candidate publication story is that an exact two-state passive model can conceal a polynomial physical state requirement for controlled prediction, even with binary sensitivity, bounded relaxation rates and fixed clock resolution. The controlled-word lower and bounded-rate upper determine this polynomial growth class. They should be presented with the unmatched exponents and the structural guarantees of the predictors, with the classical graph, logarithm-series, interpolation and rank ingredients explicitly acknowledged.

The main remaining mathematical questions are the optimal bounded-binary exponent and whether a polynomial reversible upper bound holds. The general bounded-sensitivity upper bound can also be improved, but it concerns a broader class. The different available upper bounds do not establish an intrinsic penalty for reversibility.
