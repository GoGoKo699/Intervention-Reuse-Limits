...
The constant-step upper can preserve reversibility, irreducibility, stationary zero-field preparation, the exact passive path law and static mean, and a uniform full-generator spectral band. Its field dependence is unrestricted and need not obey the target's exponential rate rule. For switching, a new bounded transition-rate density assumption permits polynomial reversible compression inside the original field-rule family. With a spectral cap alone, the available reversible upper is singly exponential and polynomial sufficiency remains open. The broader bounded-sensitivity family still has a double-exponential reversible existence bound. The sharp rank-one theorem remains a resolved special case, and the cubic, path-information, and robustness results remain foundations and operational consequences.
...
The comparison uses the same broad instantaneous-field competitor class and the actual-mean norm for the two intervention tasks. The step upper additionally preserves the stated structure, but does not impose continuity or analyticity of field dependence, the original field rule, or a bound on the description complexity of its field functions. It does not claim that any fitted constant-step model predicts switching accurately. The shift-register witnesses themselves have the logarithmic step upper and polynomial switching lower.
...
|---|---|---|
| Can all constant steps remain inexpensive while switching is hard? | [One common reversible step predictor](CONSTANT_STEP_COMPRESSION.md) needs only $\Theta(\log(1/\delta))$ states in the same bounded binary class. | Field functions are unrestricted; the original actuator rule is not imposed by this upper. |
...
| Is there a polynomial reversible sufficient condition for switching? | [Bounded transition-rate density](BOUNDED_DENSITY_REVERSIBLE_COMPRESSION.md) gives $C\delta^{-2(1+p_L)}\log^3(2/\delta)$ states inside the original field-rule family. | The density bound is stronger than a spectral cap; only the new internal cap $2Lk$ is guaranteed. |
