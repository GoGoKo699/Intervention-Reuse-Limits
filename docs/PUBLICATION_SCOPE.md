...
The binary uncapped theorem strengthens the case for an ambitious theoretical paper: a superpolynomial cost of ordinary reversibility persists with only two actuator values and arbitrarily fast hidden rival dynamics. The construction's rare weights, large fixed rate-to-gap ratio and arbitrary switching requirement remain substantive qualifications. The older binary theorem separately supplies a stronger growth class under a shared rate budget and fixed clock. This assessment remains conditional on external proof scrutiny and complete-theorem novelty; manuscript drafting remains on hold.

The current conclusions distinguish prediction tasks and predictor constraints.

| Question | Result | Essential limitation |
|---|---|---|
| Can a binary actuator force a penalty without a rival rate cap? | [The tagged binary family](BINARY_UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) has a polynomial unrestricted upper and reversible lower $\exp(\exp(c_H\sqrt{\log(1/\delta)}))$. | New target family with rare states and large fixed rate-to-gap ratio; arbitrary switching; exact unrestricted polynomial class uses the explicit union corollary. |
...
The main candidate publication story is that ordinary reversibility can force superpolynomially more physical states for reusable prediction even with a balanced binary actuator and unrestricted rival rates. Rare diagnostic tags, positive bounded observation, weighted transport repair and entropy connect the actual mean norm to this state-count obstruction. The nineteen-level result retains much better numerical target constants; the older binary theorem supplies a stronger fixed-budget growth law and fixed-clock witnesses. The passive/step/switching comparison and positive compression routes remain supporting results with their own assumptions.

The next priority is a superpolynomial uncapped reversibility lower at a fixed positive control clock. Both uncapped proofs currently use arbitrarily rapid switching to expose a full-dimensional physical coefficient set. The fixed-target Walsh hierarchy and qualitative fast-rate closure do not establish that worst-case comparison. The stronger uncapped lower $\exp(c\delta^{-\alpha})$, the same uncapped penalty on the older binary family, and a smaller target rate-to-gap ratio are separate open questions. The positive-semidefinite palindrome obstruction remains a restriction on a literal replacement route, not on all binary kernels or all entropy arguments.
