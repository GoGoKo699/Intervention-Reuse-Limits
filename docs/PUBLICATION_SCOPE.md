...
The [new finite construction](FINITE_REVERSIBILITY_ADVANTAGE.md) gives a 12-state equilibrium target and an exact 11-state stationary nonreversible predictor. Ordinary-reversible rivals with at most 11 total states incur error greater than $2^{-3000}$ under the specified shared interface, hidden exit cap $2k$, two fields and finite clock. This resolves small finite-instance existence, with an extremely conservative tolerance; it does not establish a useful measurement protocol. The target is a separate six-label, generally uncentered model, and the 11-state predictor is not proved generalized reversible. [Internal review](FINITE_ADVANTAGE_INTERNAL_REVIEW.md) and [source comparison](FINITE_ADVANTAGE_SOURCE_AUDIT.md) separate the controlled result from established completely positive factorization theory.
