...
The constant-step upper now preserves real-analytic field dependence as well as reversibility, irreducibility, stationary zero-field preparation, the exact passive path law and static mean, a uniform full-generator spectral band, and the universal linear and zero quadratic mean responses. It need not obey the target's exponential rate rule, and field derivatives are not uniformly bounded in tolerance. For switching, either a uniform positive density moment or a polynomial stationary-flux fragmentation profile permits polynomial reversible compression inside the original field-rule family. The local-walk class satisfies the latter condition even with diverging density moments. With a spectral cap alone on general graphs, the available reversible upper is singly exponential and polynomial sufficiency remains open. The broader bounded-sensitivity family still has a double-exponential reversible existence bound. The sharp rank-one theorem remains a resolved special case, and the cubic, path-information, and robustness results remain foundations and operational consequences.
...
The candidate publication story is that even a common model of every constant-field step can be much smaller than a model reusable under switching. This now occurs in the one-dimensional local-walk class with global refresh: passive cost two, logarithmic step cost, and polynomial switching cost, with the last obstruction surviving fixed clock resolution. In that class the polynomial growth is attainable reversibly while preserving the original field rule and spectral band. Density moments and small-cost fragmentation give two separate positive routes for broader reversible switching predictors. These statements retain unmatched exponents and precise structural assumptions, with classical random scenery, quadrature, Jacobi, convex selection, sampling, graph, logarithm-series and rank ingredients explicitly acknowledged.

The main remaining mathematical questions are the optimal bounded-binary exponents and polynomial reversible sufficiency under a spectral cap alone on general graphs. The local-walk result settles the growth class on one substantive subclass; it does not supply small-cost fragmentation for uniformly mixing local graphs. The analytic constant-step construction resolves qualitative field regularity; imposing the original field rule or uniform derivative bounds remains a separate refinement. The general bounded-sensitivity upper bound can also be improved, but it concerns a broader class. No equality of optimal reversible and unrestricted exponents is proved.
