...
The individual response, realization, dwell-time, and approximation mechanisms remain established. The more ambitious candidate now combines the sharp finite-field reuse theorem with a general distinction between complete intervention information and approximate prediction. The source comparisons below and in the audit constrain this claim; they do not certify originality. Manuscript writing remains on hold during research development.
...
The general-family extension now resolves the preceding research question in three parts.

| Question | New result | Essential limitation |
|---|---|---|
| Can any fixed response order give complete intervention information? | [Fixed-actuator hierarchy](FIXED_ACTUATOR_HIERARCHY.md): mean jets through $2d$ and path jets through $2d-1$ can agree in $d+2$ states, with the actuator fixed between models and hidden rates in one fixed band. Every nonzero step still separates the means. | Exact incompleteness; the first separating signal may shrink with $d$. No finite-error minimax lower bound follows. |
| What information is complete for driven visible paths? | [Actuator-process equivalence](ACTUATOR_PROCESS.md): precisely the stationary law of $g(X_t)$, recovered in principle from pulse-dependent no-exit probabilities. Finite actuator rank gives a matrix-return closure. | No stable noisy reconstruction, mean-only necessity, or general positive matrix-kernel realization is proved. |
| Is dimension-independent finite-field compression possible for the entire bounded-sensitivity family? | [General upper bound](GENERAL_FINITE_FIELD_UPPER_BOUND.md): yes, by actuator quantization, reversible rate regularization, and finite-word Markov realization, uniformly over all bounded protocols and horizons. | The explicit state bound is very large; surrogate reversibility and exact sensitivity variance are not retained. |

Together, these results distinguish exact identifiability from approximate prediction. An arbitrarily long finite calibration hierarchy is not universally complete even for an unchanged actuator, but bounded field and sensitivity still allow fixed-accuracy mean prediction with a finite state budget independent of microscopic size. The rank-one subclass supplies a sharp and structure-preserving answer within this broader picture. This is the stronger candidate scientific statement; it does not turn standard polynomial, Feynman--Kac, realization, uniformization, or Markov-approximation tools into new methods.

The next mathematical priority is the **optimal general-family state cost and the cost of preserving reversibility**. The existing subclass supplies a squared-logarithmic lower order for the larger target family, while the general upper construction is far larger. A useful first restricted case is two sensitivity levels without the distinguished-single-state geometry: its two-time kernel is already insufficient, so any sharp construction must retain additional information. Fixed finite actuator rank offers a separate matrix-return route, whose Markov positivity must be proved rather than assumed.

The constructor still receives a known target; the observation results concern specified two-model tests with ideal preparation and readout. No experimental platform or large simulation is required for the current theory question. Publication originality and a journal target remain separate assessments. Manuscript writing remains on hold.
