...
The [finite construction](FINITE_REVERSIBILITY_ADVANTAGE.md) and [new minimality theorem](FINITE_PREDICTOR_MINIMALITY.md) determine the minimum counts for one target: eleven states with stationarity alone, twelve with ordinary detailed balance, throughout $0\le\delta\le2^{-1360}$. The comparison retains the shared interface, hidden exit cap $2k$, two fields and original clock, with at most 520 ticks per witness. This resolves small finite-instance existence, with an extremely conservative tolerance; it does not establish a useful measurement protocol. The target is a separate six-label, generally uncentered model, and the 11-state predictor is not proved generalized reversible. [Internal review](FINITE_ADVANTAGE_INTERNAL_REVIEW.md) and [source comparison](FINITE_ADVANTAGE_SOURCE_AUDIT.md) separate the controlled result from established completely positive factorization theory.

The [source supplement](FINITE_PRECISION_SOURCE_AUDIT.md) identifies direct prior examples of the exact core matrix, including Fawzi–Parrilo Eq. (55). Neither that matrix nor its positive-rank gap is a novelty claim. The candidate contribution remains the controlled Markov realization and quantitative same-interface comparison. The [stronger Gram bound](FINITE_GRAM_ROBUSTNESS.md) and [selector/reinjection diagnosis](FINITE_OBSERVATION_BOTTLENECK.md) improve the research record, but do not establish useful experimental precision or a practical protocol menu.

The [single-force interpretation](SINGLE_FORCE_CONFORMATIONAL_MODEL.md), [physical-source assessment](PHYSICAL_REALIZATION_SOURCE_AUDIT.md), [uniform perturbation bounds](PHYSICAL_INTERFACE_ROBUSTNESS.md) and [even-observation closure](PHYSICAL_REVERSAL_REALIZATION.md) fix an ordinary-parity conformational model class. They do not supply a microscopic molecular realization. Small rate errors and finite ramps have a quantitative accuracy floor; arbitrary hidden control dependence is not resolved.

The current distinction is between ordinary detailed balance and generalized time reversal on a counted memory representation. On the same equilibrium targets, under the same hidden exit cap $3k$ and two-field fixed-clock task, unrestricted and generalized-reversible predictors have polynomial state growth, while ordinary reversible predictors have exponential growth in a positive power of inverse error:
