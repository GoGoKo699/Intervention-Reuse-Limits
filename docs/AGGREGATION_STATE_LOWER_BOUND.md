...
The six fixed actuator values are intentional. No binary-alphabet reduction of this static architecture comparison is claimed. The separate [binary theorem](BINARY_REVERSIBILITY_LOWER_BOUND.md) changes the dynamics and rate budget to constrain every reversible rival, rather than just this aggregation architecture. The large exponent and constants are sufficient bounds, not practical state budgets. Constructors know the supplied target; learning, parameter precision and efficient computation are outside the result.
