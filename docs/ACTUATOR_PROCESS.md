...
When $q=1$, the matrix kernel is scalar and the canonical subclass in the [finite-field theorem](FINITE_FIELD.md) is recovered. For larger fixed $q$, the equations give exact closure, but this note does not provide a sharp constructive compression through this matrix kernel. Although reversible matrix-return kernels have positive-semidefinite spectral measures, generic matrix quadrature or block-Jacobi realization need not produce a generator with nonnegative off-diagonal entries. That positivity and physical-realization step requires a separate argument. The subsequent [general reversible upper theorem](REVERSIBLE_GENERAL_COMPRESSION.md) proves dimension-independent approximation by a different construction, with a nonsharp count. A finite number of actuator levels alone does not even bound $q$; the [general lower bound](GENERAL_CONTROLLED_LOWER_BOUND.md) quantifies the larger cost already for binary sensitivities.
