...
The lower bound holds against arbitrary finite Markov competitors, including irreversible ones. It does not establish an intrinsic reversibility cost. Its intersection with the separately proved typical-label upper event places hard targets inside a class with polynomial reversible compression; the polynomial exponents are not matched. The general spectral-cap-only reversible problem remains open.
