...
**Planning record, 23 September 2026.** This document organizes the research needed before drafting. It contains no manuscript draft or journal recommendation. The preceding published baseline is commit `33f23b7`, with thirty-two mathematical verifiers. Current proof and verification updates are recorded separately below and in the verification record. Proofs, generated checks and source comparisons have different evidentiary roles; the [ledger](CLAIM_LEDGER.md) keeps them separate. Subsequent work must retain its own status and provenance.

## 1. Recommended scientific focus and result hierarchy

The principal candidate is the [fixed-clock uncapped binary theorem](FIXED_CLOCK_UNCAPPED_REVERSIBILITY_LOWER_BOUND.md). For every fixed clock $a/k>0$, the prescribed rescaled tagged family has polynomial sufficient prediction size without reversibility and reversible necessity at least $\exp(\exp(c_a[\log(1/\delta)]^{1/12}))$. Two fields, $0,H$, suffice. Both classes retain the original kinetic rule, exactly balanced binary histogram, preparation and readout; neither has a rival rate cap or lower-gap requirement. Rivals may invent new states.

The target uses rare diagnostic states and a very large fixed rate-to-gap ratio. Its hidden rates are scaled by the fixed factor $\eta=(e^{(\gamma-1)H}-e^{(-\gamma-1)H})/10^9$. Its positive gap is uniform in target size, but is not normalized to $k$. The lower has a finite witnessing horizon: at target index $n$, at most $C_a(n+1)^{12}$ clock ticks suffice at error $e^{-C_a(n+1)^{12}}$. Neither this horizon nor the state bound charges the number of mean experiments or their precision.

The precise unrestricted claim matters. On this two-field tagged-family task the theorem establishes a **polynomial upper**; it does not assert a polynomial lower. Explicitly adjoining the older binary target family gives polynomial unrestricted growth when the menu includes the five fields of the older rank proof. That separate corollary must not be silently substituted into the two-field principal comparison. The available reversible upper remains singly exponential in a positive power of inverse error, so reversible growth is unmatched.

The [earlier binary uncapped theorem](BINARY_UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) retains the stronger inner exponent $1/2$ under arbitrary rapid switching on its original tagged family. The [nineteen-level uncapped theorem](UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) retains target band $[k,3k]$ and polynomial unrestricted growth on its own family. The [older binary fixed-budget theorem](BINARY_REVERSIBILITY_LOWER_BOUND.md) retains a singly exponential inverse-error lower and fixed-clock guarantee under a rival exit cap. These scopes remain complementary.

| Role | Results to retain | Reason for inclusion |
|---|---|---|
| Principal comparison | Two-field fixed-clock uncapped tagged-family penalty | Removes the rival rate cap and rapid-switching requirement while preserving the original mean interface. |
| Observation mechanism | [Gram recovery](FIXED_CLOCK_GRAM_OBSERVABILITY.md); [positive resolvent calculus](FIXED_CLOCK_POSITIVE_RESOLVENT_CALCULUS.md) | Target-sensitive remainder norms transfer positive hidden operations from sampled means without a global rival spectral cap. |
| State mechanism | Rare-tag selectors; weighted whole-word repair; stationary word-chain upper | Connects scalar tests to entropy on arbitrary rival states; retains explicit normalization and counting. |
| Complementary quantitative scopes | Arbitrary-switching binary and tight-band nineteen-level uncapped results; older fixed-budget binary theorem | Keeps the stronger inner exponent, smaller target band and stronger rate-capped law separate. |
| Complementary task comparison | Passive / all constant steps / switching; local-path version | Shows that the experiment menu itself changes state cost. |
| Structural controls | Fresh reversible versus aggregation; density, fragmentation and expander compression | Identifies where reversible prediction remains polynomial or a restricted architecture is costly. |
| Foundations and diagnostics | Response, rank-one closure, actuator-process equivalence, rate tradeoff and qualitative closure | Supplies the model and operational limits with separate task definitions. |

The next quantitative priority is to sharpen the fixed-clock accuracy allocation, whose exponent $12$ is deliberately conservative. A smaller target rate-to-gap ratio, an uncapped lower on the older binary family, robustness to rule/histogram perturbations, and the stronger uncapped $\exp(c\delta^{-\alpha})$ law remain separate research questions. Manuscript drafting remains the last step.
...
For the fixed-clock principal family, start from the frozen tagged construction, assign $+\gamma$ to the core/corridor and $-\gamma$ to tags/ballast, and scale hidden rates by the specified $\eta>0$. With the original constants $B=5\times10^8$ and $g_*=(\sqrt{1+10^{-6}}-1)^2/[100(1+10^{-6})]$, its cap is $\eta Bk$ and its hidden band is $[\eta g_*k,2\eta Bk]$. The gap is uniform in $n$; it can be extremely small relative to the external rate. Normalizing it to $k$ would change the natural-killing argument and is not part of this theorem.

The [complete fixed-clock proof](FIXED_CLOCK_UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) combines $O(n)$ normalized logical factors, heat-polynomial degree $O((n+1)^4)$, at most $O((n+1)^5)$ resolvent factors, and physical-clock cutoffs $M=O((n+1)^6)$ and $J=O((n+1)^7)$. Both the logarithmic noise amplification and required clock horizon are $O((n+1)^{12})$. The exact positive operators remain on the rival's own states; signed approximations recover only their scalar tests. [Internal review](FIXED_CLOCK_INTERNAL_REVIEW.md) records independent checks within this investigation.

The following arbitrary-switching comparison retains its own original scaling and stronger inner exponent. For the new binary tagged family, let $B=5\times10^8$ and $g_*=(\sqrt{1+10^{-6}}-1)^2/[100(1+10^{-6})]$. Targets have exit cap $Bk$ and relaxation band $[g_*k,2Bk]$. Rescaling hidden rates by the fixed factor $1/g_*$ gives a unit-gap convention with a correspondingly larger fixed exit cap. Rival classes have no rate cap or required lower gap. The shared original rule, preparation/readout and exact balanced binary histogram remain essential.
...
| Exit budget / spectral cap, when imposed | Stabilizes the fixed-clock generator recovery. The older binary proof also needs $P=I+K/(6580k)$ entrywise nonnegative to preserve positive words. | The uncapped proofs replace raw generator recovery by bounded resolvent or mixed killed-semigroup words; the new target-sensitive proof additionally respects a fixed clock, with a weaker inner exponent. |
...
A Schur complement of the full physical resolvent isolates the hidden killed resolvent. The two fields resolve the two binary label insertions through the resolvent identity. A proved Gevrey/Chebyshev estimate approximates killed heat by a polynomial in its resolvent, with degree quadratic in logarithmic precision. Natural killing on the scaled target reproduces the earlier rare-tag filter. The [Gram lemma](FIXED_CLOCK_GRAM_OBSERVABILITY.md), [positive calculus](FIXED_CLOCK_POSITIVE_RESOLVENT_CALCULUS.md), and [full theorem](FIXED_CLOCK_UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) account for each transfer separately.
...
| Positive hidden calculus | [Schur complement, binary insertions and heat](FIXED_CLOCK_POSITIVE_RESOLVENT_CALCULUS.md) | Isolates exact positive operators on every rival and implements natural killing on the scaled target. |
| Fixed-clock conclusion | [Two-field uncapped theorem](FIXED_CLOCK_UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) | Accuracy $e^{-C_a(n+1)^{12}}$ forces $2^{3\cdot2^n/4}$ states using at most $C_a(n+1)^{12}$ ticks. |
...
The unrestricted rank lower and reversible entropy lower are separate branches. The binary uncapped theorem uses the older rank result only for its explicitly enlarged-family corollary; the tagged-family penalty already follows from the polynomial upper and new entropy lower. The aggregation norm-loss proof and exact-prefix peripheral-spectrum proof are supporting results, not prerequisites for the intrinsic binary lower.
...
The baseline binary verifier constructs 1,795 hidden vertices sparsely, with 1,796 total physical states; its new dense algebraic fixtures have dimension at most five. The baseline full suite has maximum dense dimension 68. The nineteen-level verifier constructs the 146-state width-one model sparsely. The new [binary uncapped verifier](../scripts/verify_binary_uncapped_observability.py) and [report](../reports/binary_uncapped_observability.json) add exact small PSD-boundary, mixed-coefficient, normalized-selector, weighted-repair and parameter-budget fixtures. They do not construct the full rare-tag target or prove its all-width estimates by enumeration. These are bounded certificates, not large simulation evidence. The [fixed-clock verifier](../scripts/verify_fixed_clock_observability.py) and [saved report](../reports/fixed_clock_observability.json) add finite checks of sampled-row recovery, adjoints, Schur and binary resolvent identities, contraction remainders, and the polynomial budget. Their finite fixtures do not prove the universal analytic estimate. See [Verification](VERIFICATION.md) for complete provenance and checkpoint-specific limitations.
...
| Fixed-clock uncapped superpolynomial penalty | **Resolved on the prescribed scaled binary tagged family** | The [two-field theorem](FIXED_CLOCK_UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) gives inner exponent $1/12$ with a finite horizon. Sharpening this conservative exponent is the next quantitative priority. |
...
| Fixed-clock principal equation set | Two-field polynomial upper and superpolynomial reversible lower; $e^{-C_a(n+1)^{12}}$ accuracy and $O_a((n+1)^{12})$ ticks | [Fixed-clock theorem](FIXED_CLOCK_UNCAPPED_REVERSIBILITY_LOWER_BOUND.md), [Gram lemma](FIXED_CLOCK_GRAM_OBSERVABILITY.md), and [calculus](FIXED_CLOCK_POSITIVE_RESOLVENT_CALCULUS.md). Keep the prescribed rate scaling, exact constraints and large constants beside the bounds. |
| Binary uncapped principal equation set | Tagged-family polynomial upper versus superpolynomial reversible lower; explicit union corollary; $e^{-C(n+1)^2}$ accuracy allocation | [Binary uncapped theorem equations (4), (17)–(23)](BINARY_UNCAPPED_REVERSIBILITY_LOWER_BOUND.md). Keep rare weights, the large fixed target rate-to-gap ratio and arbitrary-switching limitation beside the bound. |
| Tight-band uncapped comparison | Nineteen-level polynomial unrestricted growth versus superpolynomial reversible lower | [Nineteen-level uncapped theorem](UNCAPPED_REVERSIBILITY_LOWER_BOUND.md); [bounded-control observability](BOUNDED_CONTROL_RESOLVENT_OBSERVABILITY.md). It retains the band $[k,3k]$. |
...
| Physical construction schematic | Rare private binary tags and distant ballast, with nineteen-level ports/hub and older binary corridors as comparisons | Principal construction notes and exact reports. Draw a schematic only after choosing which construction to foreground; show omitted repeated copies explicitly and keep total-state formulas visible. |
| Main comparison table | Shared constraints and polynomial sufficient count versus superpolynomial reversible necessity, with fixed-budget comparisons | Section 4 above and ledger R15–R17, R10–R13, R8 and P1–P5. Keep the tagged family and explicit union separate. |
