...
**Planning record, 23 September 2026.** This document organizes research before drafting. PRL remains the selected target; no manuscript is drafted here. The preceding published baseline is `7209621`, with thirty-seven mathematical verifiers. R31–R33 in the [ledger](CLAIM_LEDGER.md) add exact minimum counts at improved finite precision, a stronger Gram obstruction, and limits of the present observation method. One new bounded verifier supplements these analytic derivations. Full-suite and remote-CI evidence is recorded in [Verification](VERIFICATION.md); neither internal checks nor source comparisons certify external validation or novelty.
...
The [new finite example](FINITE_REVERSIBILITY_ADVANTAGE.md) supplies a distinct, fully specified 12-state equilibrium target and an exact 11-state stationary nonreversible predictor. The [minimality refinement](FINITE_PREDICTOR_MINIMALITY.md) gives exact minimum counts $D_{\rm all}=11$ and $D_{\rm ord}=12$ for $0\le\delta\le2^{-1360}$ in the same kinetic interface with hidden exit cap $2k$. The lower uses fields $0,\log2$, clock $(\log2)/(8k)$ and at most 520 ticks per witness. The target has six kinetic labels and is generally uncentered; it is not a member of the original nineteen-level family. Its 11-state minimum is unrestricted, with no claimed generalized reversal. The underlying core matrix is itself a prior example; the [source supplement](FINITE_PRECISION_SOURCE_AUDIT.md) gives exact source locations and distinguishes the controlled realization from established positive-rank theory. A [Gram refinement](FINITE_GRAM_ROBUSTNESS.md) permits intermediate error $1/1500$, while the [observation diagnosis](FINITE_OBSERVATION_BOTTLENECK.md) proves minimum selector degree ten and supplies bounded rational alternatives whose stable recovery remains open.
