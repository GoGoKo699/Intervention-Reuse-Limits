...
The principal candidate is the [balanced binary reversibility theorem](BINARY_REVERSIBILITY_LOWER_BOUND.md): on one explicit family, requiring ordinary reversibility changes the worst-case physical state growth from polynomial to singly exponential in a positive power of inverse controlled-mean error. Both predictor classes retain the same kinetic field rule, exact binary actuator histogram and fixed outgoing-rate budget. Rivals may construct new states. The [nineteen-level theorem](DYNAMIC_LAMP_REVERSIBILITY_LOWER_BOUND.md) supplies the simpler mechanism at the smaller budget and narrower band.

The eventual central theorem should state the shared constraints before the growth comparison. Its proof components are positive observation words, quantitative recovery from actual means, stationary transport repair on the rival's states, and deterministic lamp decoding. The source audit treats the full quantitative connection as the candidate contribution; its individual coding, normalization, transport and entropy ingredients have antecedents.

The remaining results should have explicit roles:

| Role | Results to retain | Reason for inclusion |
|---|---|---|
| Principal comparison | Binary reversibility; nineteen-level version | Separates ordinary reversibility from the freedom to invent states. |
| Essential upper and lower machinery | Stationary word-chain upper; reversible prediction partition; fixed-clock whole-side rank lower; transport repair | Supplies both growth classes and connects the algebra to physical mean measurements. |
...
| Rate-dependent and qualitative extensions | State–speed–accuracy theorem; regularization; finite-state closure and nineteen-level uncapped error floor | Quantifies a varying cap and excludes arbitrarily accurate uncapped prediction below a fixed state threshold; a quantitative uncapped inverse-error law remains open. |

This is an organizational choice, not a novelty or publication-level assessment. A concise eventual paper can center the intrinsic reversibility comparison and place the other results in supporting material or separate work. Independent review remains a recommended evidence task.
...
| Exit budget / spectral cap | Bounds the microscopic clock and stabilizes quantitative generator recovery. The binary proof also needs $P=I+K/(6580k)$ entrywise nonnegative to preserve positive word operators. | Removing the cap requires more than repairing only the logarithm-transfer estimate. |
...
### Uniform cap dependence: the new quantitative extension
...
The [source audit](PRIOR_ART.md) records located primary statements, versions and access depth. It already attributes kinetic nonlinear response, Hankel/word rank, positive realization, stationary finite-order approximation, quadrature, Jacobi realization, lamp/Følner growth, conditional entropy, binary input coding, Doob transforms and marginal repair. The [rate-boundary follow-up](RATE_BOUNDARY_PRIOR_ART.md) adds primary-full-text comparisons, including spectral and singular-limit references, and distinguishes ordinary detailed balance from alternative meanings of reversible representation. It also checks why averaging a fast block with mixed actuator labels generally leaves the specified exponential field family. The candidate claim is the constrained quantitative connection in the full theorem, not an assertion that its ingredients are new. This dossier indexes those audits; it does not expand their search-coverage claims.

A theorem-level comparison of the complete binary statement with the closest reversible/positive realization and controlled stochastic realization results, plus an independent reading of the positive-selector, normalization, mixed-norm repair and all-clock transfer arguments, would strengthen the evidence before making publication-priority or independently validated proof claims. Current internal proof reviews and bounded source searches do not supply an external proof audit or priority certification. These are recommended follow-up tasks, not additional user-imposed prerequisites for equipping the repository or beginning a later draft; further numerical examples cannot substitute for that kind of review.

| Open boundary | Current status | What would resolve it |
|---|---|---|
| Quantitative uncapped exponential state law | Open; a fixed-target/fixed-budget nineteen-level positive error floor is now [proved](FINITE_STATE_FAST_RATE_CLOSURE.md) | Quantify the floor strongly enough in target width/state budget, or supply another uniform argument. The corresponding binary extension is also not established. |
| Quantify a varying cap | [Uniform nineteen-level tradeoff proved](STATE_SPEED_ACCURACY_TRADEOFF.md), with a separate internal audit | Retain cap-dependent clock requirements, the weaker fixed-clock formula and finite-evidence scope; no uncapped lower follows. |
