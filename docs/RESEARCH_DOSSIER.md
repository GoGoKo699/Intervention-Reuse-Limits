...
**Planning record, 24 September 2026.** The user asks that strength come with simplicity. The current lead is a conditional-covariance test on a familiar conformational pair: four endpoints from two equilibrium preparations distinguish three stationary predictive states from four ordinarily reversible states. The added preparation resource, a nearly sharp occupation-gap bracket, calibration conditions and endpoint costs are explicit. The earlier one-preparation, matrix, variance and asymptotic results retain their own assumptions. PRL remains the research target, readiness is not claimed, and manuscript drafting remains deferred. See [Verification](VERIFICATION.md) for local and separate remote evidence; internal review is not external validation or priority certification.
...
The [four-endpoint theorem](FAMILIAR_SWITCH_PREPARATION_WITNESS.md) makes the explanation and measurement shorter by adding access to high-field equilibrium preparation. The four cells are $(0;H),(H;0),(H;0H),(0;H0)$, with preparation before the semicolon and chronological fields after it. One shared model must explain all four using its two Gibbs-related stationary laws. The nominal low-field law is balanced. For measured spin means $(m,a,b,\ell)$, every ordinary rival with at most three states obeys at least one identity
...
The [exact-only snapshot corollary](FAMILIAR_SWITCH_PREPARATION_WITNESS.md) offers a different resource trade: use only the two opposite-order protocols $0H,H0$, both prepared in $\pi_0$, and record a noninvasive initial binary readout as well as the final one. The error norm is the maximum TV distance between the two initial/final joint probability tables. Every ordinary rival with at most three states has error $>1/1000$; the same unrestricted three-state model matches both joint laws exactly. Minimum counts are therefore three and four for $0\le\delta_{\rm joint}\le1/1000$. Exact equilibrium preparation and tilt are required. These correlated joint observations have a different sampling model, so the calibrated endpoint budgets above do not transfer; no complete multitime path-law claim follows.
...
The four-cell witness gives a short covariance mechanism and a nearly determined finite margin by adding high-equilibrium preparation. Its smaller sufficient sample budget must be compared with the earlier design at that changed resource level. The next priorities are supportable preparation, statistical efficiency and a larger observable effect under explicitly counted protocols or readouts. A microscopic device realization, broader physical interfaces and continued source comparison remain separate tasks. Arbitrary interfaces, unrestricted control-dependent hidden dynamics, matching entropy-production frontiers and the older-family binary uncapped problem remain open. Manuscript drafting remains the last step.
