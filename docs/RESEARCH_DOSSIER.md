...
**Planning record, 23 September 2026.** This document organizes the research needed before drafting. PRL is the provisional research target, as recorded in [PRL exploration](PRL_EXPLORATION.md); no manuscript is drafted here. The preceding published baseline is commit `04d34df`, with thirty-three mathematical verifiers. The new [thirty-fourth verifier](../scripts/verify_prl_exploration.py) and [saved report](../reports/prl_exploration.json) supply 561 bounded checks at dense dimension at most five. Full-suite and remote-CI status are recorded in [Verification](VERIFICATION.md). Proofs, finite checks and source comparisons have different evidentiary roles, recorded separately in the [ledger](CLAIM_LEDGER.md) and [verification record](VERIFICATION.md).

## 1. Recommended scientific focus and result hierarchy

The principal candidate is now the [tight-band fixed-clock theorem](TIGHT_BAND_FIXED_CLOCK_REVERSIBILITY.md) on the original nineteen-level targets. Their hidden relaxation band remains $[k,3k]$ and their count remains $18r2^r+2$, with $r=2^n$. At every fixed positive clock, two fields $0,H$ suffice for a polynomial unrestricted sufficient count and reversible necessity
...
Both classes retain the original rule, preparation and readout. Rivals may invent states, use arbitrary finite rates and choose any sensitivities in $[-9/100,9/100]$, without matching the target alphabet, histogram or moments. No rival lower gap is required. The target has no new tags or rate rescaling. At target index $n$, the finite witness uses $O_{a,H}((n+1)^5)$ clock ticks at error $e^{-C_{a,H}(n+1)^5}$; neither bound charges the number or precision of mean experiments.

The two-field statement gives an unrestricted **upper only**. Enlarging the menu to the thirty-nine fields $jH/38$ supplies polynomial unrestricted growth on the **same target family**, without a union. Reversible growth remains unmatched. These menu distinctions belong beside any headline comparison.

A second main result supplies a quantitative physical resource bridge. [Uniform same-state reversibilization](ENTROPY_PRODUCTION_REVERSIBILIZATION.md) bounds the controlled-mean change by $R^{3/2}\sqrt{\sigma_0/k}$, where $R=e^{(1+G)H}$ and $\sigma_0$ is full zero-field stationary entropy production under ordinary identity reversal. It requires no cap or minimum stationary mass. Consequently the [state–entropy-production tradeoff](STATE_ENTROPY_PRODUCTION_TRADEOFF.md) gives
...
The broad class inherits the new fifth-root lower. The stronger $\exp(c\varepsilon^{-\alpha})$ frontier retains the older common exit cap $3k$ and exact histogram; it cannot be promoted to the broad class. A same-state perturbation of the word-chain upper gives polynomial prediction with finite $\sigma_0=O(k\log(1/\delta))$. These bounds do not match and are not a proved heat cost.

| Role | Results to retain | Reason for inclusion |
|---|---|---|
| Principal comparison | [Original tight-band fixed-clock theorem](TIGHT_BAND_FIXED_CLOCK_REVERSIBILITY.md) | Removes rival rate and histogram restrictions with two physical fields and unchanged target band. |
...
| Physical resource bridge | [Uniform reversibilization](ENTROPY_PRODUCTION_REVERSIBILIZATION.md); [state–entropy-production tradeoff](STATE_ENTROPY_PRODUCTION_TRADEOFF.md) | Relates state, fidelity and stationary path irreversibility on the same predictor states. |
...
The next priority is a precise complete-theorem source comparison and physical-generality assessment, followed by a modest certified finite-precision instance or a broader kinetic class. Sharpening the fifth-root exponent, matching the entropy-production frontier, the older-family binary uncapped problem and arbitrary alternative field rules remain open. Manuscript drafting remains the last step.
...
The principal target is exactly the original nineteen-level lamp family, with zero-level hidden mass $1/2$ and each nonzero level $\pm j/100$ of mass $1/36$. Its target band is $[k,3k]$. Rivals need only a stationary hidden law, the original interface and sensitivities in the same bounded interval; their rates, histogram and number of levels are unrestricted. The positive filters discriminate the finite target grid, while remaining nonnegative on the whole rival interval.
...
| Actuator range and histogram | The new positive selectors require only rival sensitivities in $[-9/100,9/100]$; the finite grid and histogram belong to the target. | Exact histogram remains an assumption of the older stronger capped frontier. Arbitrary changes of the field rule are not covered. |
...
Arithmetic reversibilization $(K+K^*)/2$ preserves the stationary law, labels and every exit rate. A common reset to the visible state bounds endpoint relative entropy uniformly in time, giving mean error $R^{3/2}\sqrt{\sigma_0/k}$. This proves the resource comparison before taking the worst-case target supremum. The uncapped no-histogram frontier and the stronger capped exact-histogram frontier retain distinct competitor classes. Ordinary path entropy production is not identified with stationary Shannon entropy, protocol heat or generalized reversal of odd variables.
...
| Rival histogram and field-rule freedom | Histogram and finite-alphabet promises removed within the fixed sensitivity interval | Arbitrary alternative kinetic field rules remain open; the new positivity argument does not cover unbounded sensitivities. |
| State–fidelity–entropy-production frontier | Quantitative broad and capped frontiers proved; finite-EPR polynomial upper | Match the frontier or establish a broader thermodynamic realization; no heat-cost identification follows automatically. |
...
| Fixed-clock principal equation set | Two-field polynomial upper and fifth-root reversible lower; unchanged $[k,3k]$ target; fifth-power clock budget | [Tight-band theorem](TIGHT_BAND_FIXED_CLOCK_REVERSIBILITY.md), [selectors](POSITIVE_LABEL_SELECTORS.md), and [Gram lemma](FIXED_CLOCK_GRAM_OBSERVABILITY.md). Separate the thirty-nine-field unrestricted growth corollary. |
| Entropy-production equation set | Same-state mean bound, broad/capped resource frontiers, finite-EPR upper | [Reversibilization](ENTROPY_PRODUCTION_REVERSIBILIZATION.md) and [state–entropy-production tradeoff](STATE_ENTROPY_PRODUCTION_TRADEOFF.md). Keep full-zero versus hidden normalization and ordinary-reversal convention explicit. |
...
| Main comparison table | Shared constraints and polynomial sufficient count versus superpolynomial reversible necessity, with fixed-budget comparisons | Section 4 above and ledger R18–R21, with R15–R17 and P1–P5 as scoped comparisons. Keep two-field/39-field menus and broad/capped frontiers separate. |
