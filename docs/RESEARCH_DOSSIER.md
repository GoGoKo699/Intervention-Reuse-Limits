...
**Planning record, 23 September 2026.** This document organizes research before drafting. PRL remains the selected target; no manuscript is drafted here. The preceding published baseline is `a60dee5`, with thirty-eight mathematical verifiers. R34–R35 in the [ledger](CLAIM_LEDGER.md) add a stable finite observation certificate and a complementary nine-state reversible approximation. Two new bounded verifiers supplement the analytic proofs; the exact rational target-basis certificate is an essential computer-assisted premise of R34. Full-suite and remote-CI evidence is recorded in [Verification](VERIFICATION.md); internal checks and source comparisons do not certify external validation or novelty.
...
| Useful finite precision | Open after an explicit small state-gap certificate | R34 certifies ordinary minimum twelve at $2^{-220}$ on 12,766 experiments of at most 66 ticks; R35 supplies nine reversible states at $1/2376$. Sharper lower/upper bounds or a stronger target are needed. Neither theorem supplies a sample budget. |
