...
**Planning record, 24 September 2026.** The user asks that strength come with simplicity. The current lead is a familiar pair of interacting conformational switches: three stationary predictive states suffice, while ordinary detailed balance requires four. The calibrated seven-experiment advantage now has explicit preparation, control and measurement budgets. Those budgets make a larger simple observable gap or a more efficient experiment the next scientific target. The earlier matrix, variance and asymptotic results remain supporting comparisons with their own assumptions. PRL remains the research target, readiness is not claimed, and manuscript drafting remains deferred. See [Verification](VERIFICATION.md) for local and separate remote evidence; internal review is not external validation or priority certification.
...
|---|---|---|
| Familiar conformational mechanism | [Two-switch structure](FAMILIAR_SWITCH_STRUCTURE.md); [seven-mean certificate](FAMILIAR_SWITCH_FINITE_MARGIN.md) | Three-versus-four counts at occupation tolerance $1/2000$, with seven short experiments and no rival rate cap; same-menu ordinary-three-state upper below $1/1000$. |
| Calibration and measurement | [Calibration theorem](FAMILIAR_SWITCH_CALIBRATION.md); [measurement cost](FAMILIAR_SWITCH_MEASUREMENT_COST.md) | Actual three-versus-four counts on $10^{-5}\le\delta_P\le3/10000$ under the stated guarantees; conservative sufficient endpoint budgets and a separate adaptive lower bound expose the experimental limitation. |
...
The familiar two-switch example now has a certified finite margin, explicit calibration allowances and endpoint sample bounds. The next priority is a larger simple observable gap, a better protocol or readout, or greater statistical efficiency. Improving calibration bookkeeping alone does not resolve the present measurement burden. A microscopic device realization, broader physical interfaces and continued source comparison remain separate tasks. Arbitrary interfaces, unrestricted control-dependent hidden dynamics, matching entropy-production frontiers and the older-family binary uncapped problem remain open. Manuscript drafting remains the last step.
...
The [switch-calibration verifier](../scripts/verify_switch_calibration.py) and [saved report](../reports/switch_calibration.json) support the latest calibration and measurement-cost checkpoint. Their final check count, proof bindings, full-suite completion and separate remote evidence are recorded in [Verification](VERIFICATION.md); the earlier counts above remain historical.
...
| Useful finite precision | R39 certifies the small nominal gap; R40 makes it calibration tolerant under stated assumptions | The actual seven-word occupation gap exceeds $3/10000$, but R41's endpoint costs are substantial. Seek a larger simple observable gap, a better protocol/readout or more efficient inference. R34–R37 retain their distinct earlier precision and compression bounds. |
| Arbitrary expander labeling | Open | A uniform compression theorem beyond the stated good-label event. |
| Inference, sample cost, practical construction | R41 now treats the seven-word fresh-endpoint task; sufficient and necessary counts remain far apart | Improve statistical efficiency or the observable separation. No endpoint lower bound for richer trajectory observations, physical reset implementation or practical feasibility is established. |
