...
**Planning record, 23 September 2026.** This document organizes the research needed before drafting. PRL is the provisional research target, as recorded in [PRL exploration](PRL_EXPLORATION.md); no manuscript is drafted here. The preceding published baseline is commit `e38dde1`, with thirty-four mathematical verifiers. Five new proof notes, indexed as R22–R26 in the [ledger](CLAIM_LEDGER.md), have passed separate internal mathematical reviews. The new [thirty-fifth verifier](../scripts/verify_kinetic_parity.py) and [saved report](../reports/kinetic_parity.json) supply bounded kinetic-interface, reset and reversal checks. Full-suite and remote-CI status are recorded only in [Verification](VERIFICATION.md). Proofs, finite checks and source comparisons have different evidentiary roles.

## 1. Recommended scientific focus and result hierarchy

The principal candidate is now the [kinetic-interface and reversal comparison](KINETIC_PARITY_RESOURCE_TRADEOFF.md) on the original nineteen-level targets. Their hidden relaxation band remains $[k,3k]$ and their count remains $18r2^r+2$, with $r=2^n$. With a common hidden exit cap $3k$, every fixed positive clock and the same two fields $0,H$ give
...
The kinetic-rule question is partly resolved by arbitrary bounded barriers sharing the specified block tilt and zero-field normalization. General control-dependent hidden dynamics and arbitrary interfaces remain outside the result. The next priorities are a modest certified finite-precision separation and an identifiable physical realization with a specified reversal convention. Sharpening the uncapped lower, matching the entropy-production frontier and the older-family binary uncapped problem remain open. Manuscript drafting remains the last step.
...
The published `e38dde1` baseline has thirty-four verifiers, including the [PRL verifier](../scripts/verify_prl_exploration.py) and [report](../reports/prl_exploration.json) with 561 bounded selector/EPR/budget checks. The new [thirty-fifth verifier](../scripts/verify_kinetic_parity.py) and [report](../reports/kinetic_parity.json) contain 166 bounded checks at dense dimension at most nine, with hashes of all five new proof notes. They check a small generalized-reversal word chain, its physical response equivalence, kinetic selectors and the general reset constants. The parity fixture is not itself a certified small state-count advantage. Five new analytic proof notes have passed internal reviews; finite checks supplement those proofs. Full-suite, saved-report provenance and remote-CI status belong to [Verification](VERIFICATION.md), separately from mathematical review.
