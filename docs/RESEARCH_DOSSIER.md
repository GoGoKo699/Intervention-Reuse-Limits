...
**Planning record, 23 September 2026.** The user asks that strength come with simplicity. The current lead is a familiar pair of interacting conformational switches: three stationary predictive states suffice, while ordinary detailed balance requires four. The earlier matrix, variance and asymptotic results remain supporting comparisons with their own assumptions. PRL remains the research target and manuscript drafting remains deferred. See [Verification](VERIFICATION.md) for local and separate remote evidence; internal review is not external validation or priority certification.
...
The proof uses eleven endpoint means, at most five ticks and three field segments per experiment, at every prescribed positive clock. A positive error margin exists on this same menu even for unbounded rival rates; its numerical value remains unproved. The explicit three-state family serves all nonnegative-field protocols, with exits below two. This is a mean-prediction theorem, not a full binary-path realization theorem, and the target's passive path already has memory.

The [protocol and accuracy record](FAMILIAR_SWITCH_PROTOCOLS.md) provides a caution about scale: on twelve two-switch fixtures, each screened with 33 protocols, retained ordinary three-state models achieve below $0.14\%$ occupation error. These bounded numerical uppers do not certify global optimality, an all-protocol guarantee or a lower margin. The [source audit](FAMILIAR_SWITCH_SOURCE_AUDIT.md) distinguishes established kinetic/closure ingredients from the controlled state-count statement. The next quantitative task is a certified useful error margin on the eleven-word menu, with the shared Gibbs-tilt assumption explicit.
...
|---|---|---|
| Familiar conformational mechanism | [Two-switch structure](FAMILIAR_SWITCH_STRUCTURE.md); [protocol record](FAMILIAR_SWITCH_PROTOCOLS.md) | Exact three-versus-four counts from two fields and eleven short experiments; a positive but unquantified margin without a rival rate cap. |
...
The familiar two-switch example now establishes a small state advantage with eleven short experiments. The next priority is to certify a useful error margin for this example and quantify its sample cost; the existing fitted comparators are upper errors only. A microscopic device realization remains a separate question. Arbitrary interfaces, unrestricted control-dependent hidden dynamics, matching entropy-production frontiers and the older-family binary uncapped problem remain open. Manuscript drafting remains the last step.
...
| Useful finite precision | Open after an explicit small state-gap certificate | R34 certifies ordinary minimum twelve at $2^{-220}$; R35 supplies nine reversible states at $1/2376$, and R37 supplies two at $557/51920$. R36 organizes the exact mechanism but gives no useful general tolerance. A stronger target or sharper directly observable separation is needed. |
