...
**Planning record, 24 September 2026.** The user asks that strength come with simplicity. The current lead is a familiar pair of interacting conformational switches: three stationary predictive states suffice, while ordinary detailed balance requires four. The earlier matrix, variance and asymptotic results remain supporting comparisons with their own assumptions. PRL remains the research target and manuscript drafting remains deferred. See [Verification](VERIFICATION.md) for local and separate remote evidence; internal review is not external validation or priority certification.
...
The familiar two-switch example now has a certified finite margin from seven short experiments. The next priorities are quantitative robustness to preparation, field and timing calibration, the physical scope of the shared Gibbs tilt, and continued source comparison. Sample cost and a microscopic device realization remain separate questions. Arbitrary interfaces, unrestricted control-dependent hidden dynamics, matching entropy-production frontiers and the older-family binary uncapped problem remain open. Manuscript drafting remains the last step.
