...
**Planning record, 23 September 2026.** This document organizes the research needed before drafting. It contains no manuscript draft or journal recommendation. The preceding published baseline is commit `63f6be5`, with thirty-one mathematical verifiers. Current proof and verification updates are recorded separately below and in the verification record. Proofs, generated checks and source comparisons have different evidentiary roles; the [ledger](CLAIM_LEDGER.md) keeps them separate. Subsequent work must retain its own status and provenance.

## 1. Recommended scientific focus and result hierarchy

The principal candidate is now the [balanced binary uncapped theorem](BINARY_UNCAPPED_REVERSIBILITY_LOWER_BOUND.md). A new uniformly mixing tagged target family has a polynomial sufficient state count when ordinary reversibility is optional, whereas reversible predictors require at least $\exp(\exp(c\sqrt{\log(1/\delta)}))$ states in the worst case. Both classes retain the original kinetic rule, exactly balanced binary histogram, preparation and readout; neither has a rival rate cap. Rivals may invent new states. The target uses rare diagnostic states and a very large fixed rate-to-gap ratio, and the proof uses arbitrarily fast switching.

The precise unrestricted claim matters. On the tagged family, the theorem establishes a **polynomial upper**; it does not assert a polynomial lower there. Section 7 explicitly adjoins the older binary target family to obtain a common enlarged family with polynomial unrestricted growth and the same superpolynomial reversible lower. That union is stated as a corollary, not silently substituted into the principal comparison. The available reversible upper remains singly exponential in a positive power of inverse error, so the uncapped growth bounds are unmatched.

The [nineteen-level uncapped theorem](UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) remains a valuable complementary result with target band $[k,3k]$ and polynomial unrestricted growth on its original family. The [older binary theorem](BINARY_REVERSIBILITY_LOWER_BOUND.md) retains the stronger singly exponential lower under the common fixed exit budget $6580k$, and it survives every fixed positive control clock. Neither property follows for the new tagged family merely because its actuator is binary.

The uncapped binary proof combines [mixed bounded-word observation](MIXED_KILLED_WORD_OBSERVABILITY.md), [positive rare-tag selectors](BINARY_TAGGED_UNCAPPED_CONSTRUCTION.md), and [weighted whole-word repair](WEIGHTED_WHOLE_WORD_REPAIR.md). One-scalar continuation extracts positive proof operators from actual physical means; normalization and repair act on the rival's own states; deterministic lamp decoding forces high entropy. The [binary source comparison](BINARY_OBSERVATION_PRIOR_ART.md) attributes the established ingredients and isolates the combined quantitative claim without certifying priority.

| Role | Results to retain | Reason for inclusion |
|---|---|---|
| Principal comparison | Binary uncapped tagged-family penalty; explicit union corollary | Removes both the multilevel actuator and rival rate cap while preserving the original interface; distinguishes a polynomial upper from a polynomial growth class. |
| Essential mechanism | Mixed killed-word observability; rare-tag selectors; weighted whole-word repair; stationary word-chain upper | Connects actual means to positive operators and entropy on arbitrary new rival states; all approximation and normalization costs are explicit. |
| Complementary strongest scopes | Nineteen-level uncapped tight-band result; older binary fixed-budget and fixed-clock result | Keeps the much smaller target rate-to-gap ratio and the stronger growth/clock guarantees visible. |
...
| Structural controls | Fresh reversible versus aggregation; density, local-fragmentation and expander compression | Identifies where reversible prediction remains polynomial or a restricted architecture is costly. |
| Foundations and diagnostics | Cubic response, rank-one finite-field closure, actuator-process equivalence, path response, near-lumpability | Explains the model and operational limits; these are separate tasks. |
| Rate and fixed-target support | State–speed–accuracy; regularization; finite-state closure; Walsh hierarchy; binary PSD boundaries | Distinguishes rate dependence, fixed-target rank floors, and failed proof routes from the principal penalty. |

The single next research priority is a **fixed-clock uncapped superpolynomial penalty**. Stronger uncapped $\exp(c\delta^{-\alpha})$ growth, a smaller target rate-to-gap ratio, and removing the cap on the older binary family are separate strengthening questions. Manuscript drafting remains the last step.
...
| Exit budget / spectral cap, when imposed | Stabilizes the fixed-clock generator recovery. The older binary proof also needs $P=I+K/(6580k)$ entrywise nonnegative to preserve positive words. | The uncapped proofs replace raw generator recovery by bounded resolvent or mixed killed-semigroup words; they give weaker lowers and use arbitrarily fast switching. |
...
| Fixed-clock uncapped superpolynomial penalty | **Single next research priority** | Replace the arbitrary-fast-switching convexification step with a proof respecting one positive minimum dwell time. Fixed-clock polynomial rank lowers already hold. |
...
| Main comparison table | Shared constraints and polynomial sufficient count versus superpolynomial reversible necessity, with fixed-budget comparisons | Section 4 above and ledger R10–R13, R8 and P1–P5. Keep the tagged family and explicit union separate. |
