...
The strongest current claim is the [finite-field minimax theorem](FINITE_FIELD.md), including a [fixed nonzero step witness](FIXED_FIELD_LOWER_BOUND.md). The new path-information and near-lumpability comparisons are recorded below; earlier coefficient-stage comparisons remain relevant foundations.
...
The main scientific extension question is now the actuator restriction. Identical kernels with fixed actuator parameters can give different higher responses outside the rank-one subclass. Determining the extra information needed for a broader finite-field theorem is more consequential than optimizing current constants or adding numerical examples. No journal tier, molecular implementation, or independent validation is asserted.
