...
The [expanded scope](PUBLICATION_SCOPE.md) now prioritizes the actual finite-field prediction theorem. Its optimal state-growth order is necessary even for a single fixed intervention and is attainable uniformly over bounded protocols while preserving reversible Markov structure. The broader coefficient theorem remains a foundation, while the path-information and near-lumpability results supply operational statements.
...
The actuator extension now has exact information and finite-accuracy existence results. The main unresolved mathematical question is how the optimal physical state count grows for the general family, and whether a comparable upper bound can preserve reversibility. The two-level counterexample shows why its two-time kernel cannot simply replace the scalar one. No journal tier, molecular implementation, or independent validation is asserted.
