...
**Historical role.** The [positive shift-register construction](SHIFT_REGISTER_LOWER_BOUND.md) and its [polynomial continuation](POLYNOMIAL_CONTROLLED_LOWER_BOUND.md) now give stronger finite-error lower bounds, including polynomial necessary growth with a fixed switching clock. This note retains the earlier proof and its exact cubic-versus-full separation. Section 9 shows that truncating these particular weakly coupled tree targets already attains error $\exp[-\Theta(n^2)]$ below the $2^n$-state threshold, so improving their word filters alone cannot produce the stronger error scale.
