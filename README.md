...
**Research status:** a fixed nineteen-value actuator family separates reversible and nonreversible prediction itself. Under the same internal outgoing-rate budget, original field rule and exact actuator histogram, nonreversible predictors have polynomial state cost in inverse accuracy, while reversible predictors can require exponentially many states. The reversible lower permits entirely new state spaces.

The earlier bounded-rate binary target class has three different prediction costs: exactly two states passively, $\Theta(\log(1/\delta))$ states for every constant-field step, and $\delta^{-\Theta(1)}$ states for arbitrary switching. This separation holds already for one-dimensional local walks with global refresh, where a polynomial switching predictor can preserve reversibility, the original field rule and spectral band. The constant-step upper has analytic field dependence but allows a different rate rule. Polynomial reversible switching cost also holds for typical labels on connected sparse expanders with a uniform local mixing gap. The new nineteen-value result does not settle the binary cap-only question or allow unbounded-rate reversible rivals. Exponents remain unmatched. Originality is still under assessment, and manuscript writing remains on hold.

## Reversibility can have an exponential state cost
...
Here $p$ depends on the alphabet, rate cap and field bound; for the binary band above it is the displayed exponent. These upper bounds alone do not establish an additional optimal cost. The nineteen-level dynamic-lamp theorem supplies an exponential reversible lower under a common outgoing-rate budget. Whether polynomial reversible size always suffices for binary actuators remains open.
...
| Question | Document |
|---|---|
| Can reversibility itself force exponentially more states under the same rate budget? | [Dynamic-lamp reversible lower and common-family comparison](docs/DYNAMIC_LAMP_REVERSIBILITY_LOWER_BOUND.md) |
