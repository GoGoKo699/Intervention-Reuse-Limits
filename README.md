...
[Verification](docs/VERIFICATION.md) records the full suite and its limits. The new four-word experiment removes rival preparation from the small-state null while retaining the complete ideal-pair three-state fit. Its next scientific priority is a physically justified measurement and control specification: sector protection, conditional detector freshness, the shared Gibbs tilt and complete readout/reset exposure. The source comparison must also distinguish this combined realization claim from established detailed-balance reciprocity. Sample counts are sufficient bounds, not optimal costs or device feasibility claims. Manuscript drafting remains deferred.
