...
[Verification](docs/VERIFICATION.md) records the full suite and its limits. The four-word experiment removes rival preparation, and its new endpoint-registration formulation replaces exact protection and fixed symmetric errors by quantitative measurement conditions. The relative-force norm remains essential when arbitrary preparation exposes rare states. The next priority is to assess whether a concrete physical operating regime can meet the joint signal, noise, leakage and force specifications, and whether these guarantees support a clear publication claim against the close prior work. Current counts are sufficient bounds, not optimal costs or demonstrated feasibility. Manuscript drafting remains deferred.
