...
The unchanged [twelve-state equilibrium target](docs/FINITE_REVERSIBILITY_ADVANTAGE.md) has an exact **eleven-state stationary nonreversible predictor**. The [bounded rational observation theorem](docs/BOUNDED_RATIONAL_OBSERVATION_CERTIFICATE.md) proves that its **ordinary-reversible minimum is twelve** for

$$
0\le\delta\le2^{-220}.
$$

The lower uses an explicit list of **12,766 mean experiments**, only fields $0,\log2$, the unchanged clock $(\log2)/(8k)$, and at most **66 ticks** per experiment. The comparison retains the shared kinetic interface and hidden exit budget $2k$; rivals may choose arbitrary new states, masses and endpoint barriers in $[0,1]$. The target has hidden band $[k,3k]$ and an exact two-state passive visible law. Its six kinetic labels are generally uncentered and separate from the nineteen-level asymptotic family below.

The construction compares six edge-memory states of a five-vertex graph with five states that store the next probe. A stochastic intertwiner proves exact controlled equivalence. Ordinary reversibility forces a nonnegative Gram representation requiring eleven hidden states, plus the visible state. The new proof recovers bounded rational label selectors through a ten-dimensional observable basis. An exact rational, computer-assisted certificate supplies its essential conditioning bound; the transfer to arbitrary reversible rivals is analytic. The [review](docs/RATIONAL_OBSERVATION_INTERNAL_REVIEW.md), [verifier](scripts/verify_rational_observation.py), [report](reports/rational_observation.json) and [source comparison](docs/RATIONAL_OBSERVATION_SOURCE_AUDIT.md) give the details.

The eleven-state count is sufficient throughout the new interval. The [earlier minimality theorem](docs/FINITE_PREDICTOR_MINIMALITY.md) proves that eleven is also the unrestricted minimum for $\delta\le2^{-1360}$ on the full two-field clock task, already using all words through 520 ticks. That unrestricted lower is not asserted for the new smaller experiment list. The predictor is **not a proved eleven-state generalized-reversible model**; it can be regularized on the same states to finite ordinary entropy production at any positive error.

A complementary [symmetry-averaged construction](docs/SYMMETRY_AVERAGED_REVERSIBLE_COMPRESSION.md) supplies a **nine-state ordinary-reversible predictor with error at most $1/2376$**, uniformly over all two-field switching protocols and all horizons, even without the clock restriction. It retains the same hidden exit cap. Thus the new lower and this upper leave a wide unresolved accuracy range. The precision $2^{-220}$ is still impractical and gives no useful sampling guarantee.

Stable recovery of the bounded rational selectors is now proved. The [earlier observation diagnosis](docs/FINITE_OBSERVATION_BOTTLENECK.md) and [sharper $1/1500$ Gram obstruction](docs/FINITE_GRAM_ROBUSTNESS.md) remain part of the argument and its history. The original [construction review](docs/FINITE_ADVANTAGE_INTERNAL_REVIEW.md), [verifier](scripts/verify_finite_advantage.py), [source audit](docs/FINITE_ADVANTAGE_SOURCE_AUDIT.md), and [earlier precision review](docs/FINITE_PRECISION_INTERNAL_REVIEW.md) are preserved.
