...
**A fixed score reduces the unknown-detector test to 32 million paired trials.** The [new test](docs/FAMILIAR_SWITCH_UNCALIBRATED_SCORE_TEST.md) assigns a small integer score to each already recorded bit pair; it needs no extra observations or detector calibration. It gives false rejection below 5% and target power above 95% under the same physical allowances. The allocation is 18.56 million trials of $0H$ and 13.44 million of $H0$, or 64 million binary readouts. Allowing ordinary models an additional observed-joint-TV approximation error $10^{-4}$ requires a sufficient **90 million paired trials**, or 180 million readouts. That approximation allowance exceeds the general three-state constructive error $2\times10^{-5}$. The original 508-million-trial confidence-box test remains valid, but its sufficient budget is superseded by this design; none of these counts is claimed optimal.

**Detector calibration has a provable statistical value.** For the same nominal target with one-percent bit errors, the [information lower bound](docs/FAMILIAR_SWITCH_DETECTOR_INFORMATION_COST.md) requires at least **2,384,996 paired trials** for any fixed-budget test against the unknown-detector class, even though the earlier calibrated test below needs only 1.2 million. Adaptive protocol choices and stopping still require $\mathbb E_*N>810000\log19$. Thus the specified calibration promise reduces the necessary observation budget by more than a factor of 1.98 in this comparison. The cost of obtaining that calibration is separate and unpriced. The smaller calibrated-detector counts do not apply to the unknown-contrast null.
...
The combined [physical-interface verifier](scripts/verify_switch_physical_interface.py) and [report](reports/switch_physical_interface.json) retain their 108 exact checks. The new [unknown-detector cost verifier](scripts/verify_switch_uncalibrated_score.py) and [report](reports/switch_uncalibrated_score.json) certify the score and information bounds. The [statistical source comparison](docs/FAMILIAR_SWITCH_UNCALIBRATED_SCORE_SOURCE_AUDIT.md), [physical-interface review](docs/FAMILIAR_SWITCH_PHYSICAL_INTERFACE_INTERNAL_REVIEW.md) and [verification record](docs/VERIFICATION.md) distinguish analytic review, finite checks and the complete repository gate.
...
[Verification](docs/VERIFICATION.md) records the full suite and its limits. The certificates bound entire error boxes and fixed comparison models; they do not optimize over rival generators. The next scientific priorities are a larger observable separation with a comparably short protocol and independently supported preparation and initial-readout conditions for the charge candidate. The necessary sampling lower bound shows that better statistics alone cannot recover the calibrated budget on this same experiment. The remaining sampling burden and instrument assumptions are obstacles to physical significance. Manuscript drafting remains deferred.
