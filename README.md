...
The [finite construction](docs/FINITE_REVERSIBILITY_ADVANTAGE.md) and [minimality theorem](docs/FINITE_PREDICTOR_MINIMALITY.md) give an equilibrium target whose minimum predictor size is **11 total states with stationarity alone, and 12 with ordinary detailed balance**, throughout $0\le\delta\le\delta_0$. The 11-state predictor reproduces every controlled mean exactly. The comparison uses the specified shared kinetic interface and hidden exit budget $2k$, with

$$
\delta_0=2^{-1360}.
$$

Only fields $0,\log2$ are needed for the lower, at clock $(\log2)/(8k)$ and with at most 520 ticks per witness. Rivals need no target labels or histogram. The target has hidden band $[k,3k]$ and an exact two-state passive visible law. Its six kinetic labels form a new, generally uncentered target; it is separate from the nineteen-level asymptotic family below.
...
This determines both minimum counts in a small finite-accuracy example. The certified tolerance remains extremely small and gives no practical measurement or sample guarantee. A [sharper Gram obstruction](docs/FINITE_GRAM_ROBUSTNESS.md) tolerates intermediate matrix error $1/1500$, while the [observation diagnosis](docs/FINITE_OBSERVATION_BOTTLENECK.md) explains why that is not a measured-mean tolerance. It also supplies bounded rational selectors whose stable observation remains open. The [new review](docs/FINITE_PRECISION_INTERNAL_REVIEW.md), [verifier](scripts/verify_finite_minimality.py) and [source supplement](docs/FINITE_PRECISION_SOURCE_AUDIT.md) record the evidence and direct prior examples of the same core matrix. The original [proof review](docs/FINITE_ADVANTAGE_INTERNAL_REVIEW.md), [verifier](scripts/verify_finite_advantage.py) and [source audit](docs/FINITE_ADVANTAGE_SOURCE_AUDIT.md) remain preserved.
