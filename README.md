...
**Research status:** reversibility can force superpolynomial prediction cost even when hidden rates are unrestricted. On one fixed nineteen-level target family, nonreversible predictors have polynomial state cost in inverse accuracy, while reversible predictors require at least $\exp(\exp(c\sqrt{\log(1/\delta)}))$ states. Both preserve the original field rule and exact actuator histogram and may construct entirely new states. A separate balanced binary theorem gives the stronger singly exponential growth class under a common fixed rate budget.
...
The new [uncapped theorem](docs/UNCAPPED_REVERSIBILITY_LOWER_BOUND.md) combines [bounded-control resolvent observability](docs/BOUNDED_CONTROL_RESOLVENT_OBSERVABILITY.md) with positive transport repair and entropy. It uses thirty-nine fixed physical fields and arbitrarily rapid switching. It removes the rival rate cap without transferring unbounded generator moments. The stronger uncapped lower $\exp(c\delta^{-\alpha})$, a binary extension, and a fixed-clock superpolynomial separation remain open.
