...
The [new kinetic score test](docs/FAMILIAR_SWITCH_PERCENT_SCORE_TEST.md) needs **five million fresh independent trials**, 2.5 million per protocol and ten million binary readouts, for false rejection below 5% and power above 95%. Its ordinary null allows observed-joint-TV prediction error $0.0002$, exceeding the general-three-state error bound $0.000078025101$. It retains the integer scores but uses new gates and threshold .0021, with its own proof for the larger family. Preparation, the initial joint instrument, fixed plateau generators and independent symmetric electronics remain substantive promises.
...
The [percent-kinetic verifier](scripts/verify_switch_percent_kinetics.py), [report](reports/switch_percent_kinetics.json), [source comparison](docs/FAMILIAR_SWITCH_PERCENT_KINETIC_SOURCE_AUDIT.md) and [internal review](docs/FAMILIAR_SWITCH_PERCENT_KINETIC_INTERNAL_REVIEW.md) accompany the direct robustness and local realization. The earlier [kinetic-interface verifier](scripts/verify_switch_kinetic_interface.py), [report](reports/switch_kinetic_interface.json), [source comparison](docs/FAMILIAR_SWITCH_KINETIC_INTERFACE_SOURCE_AUDIT.md) and [internal review](docs/FAMILIAR_SWITCH_KINETIC_INTERFACE_INTERNAL_REVIEW.md) retain the protected-readout and integrated-error results. These records distinguish mathematical guarantees from experimental calibration.
...
[Verification](docs/VERIFICATION.md) records the full suite and its limits. The certificates cover entire error boxes and explicit local realizations; they do not optimize the allowed rate error. The next priority is to justify serial reset-and-measure sampling with finite waits and price preparation under explicit conditional promises. A finite target mixing bound alone does not establish independent repeated trials or prepare arbitrary slow rivals. The one-percent result removes the earlier very small sufficient kinetic allowance as the immediate mathematical bottleneck; a concrete device must still support the preparation, instrument, stationary-law and detector assumptions. Further field or score-constant tuning is secondary. Manuscript drafting remains deferred.
