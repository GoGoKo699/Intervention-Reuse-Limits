...
**Research status:** reversibility can force exponentially more prediction states even with a balanced binary actuator. On one common target family, nonreversible predictors have polynomial state cost in inverse accuracy, while reversible predictors have singly exponential cost in a positive power of inverse accuracy. Both use the same fixed outgoing-rate budget, original field rule and exact actuator histogram; both may construct entirely new states.
...
The [state–speed–accuracy theorem](docs/STATE_SPEED_ACCURACY_TRADEOFF.md) now tracks a growing rival spectral cap explicitly. On the nineteen-level family, it proves $\log\log D_{\rm rev}(\delta,\Lambda)\ge c_H\log(1/\delta)/\log(\Lambda+2)-C_0$. Its strongest uniform form uses pulse lengths of order $1/(k\Lambda)$; the fixed-clock version is stated separately. A [same-state rate regularization](docs/RATE_REGULARIZATION.md) gives an all-protocol approximation at cap $O_{G,H}(k\delta^{-1}\log(1/\delta))$. These two bounds do not establish the uncapped exponential state law. The [boundary analysis](docs/UNBOUNDED_RATE_REVERSIBILITY_BOUNDARY.md) identifies both a failed binary substitution route and a nineteen-level algebraic repair that no longer needs a rate cap.

The [finite-state fast-rate closure](docs/FINITE_STATE_FAST_RATE_CLOSURE.md) also yields a qualitative uncapped result. For each fixed nineteen-level target of width $n$ and hidden-state budget $D<2^{2^n}$, the infimum mean error over reversible original-rule predictors with the same histogram, preparation and readout is strictly positive, even with arbitrarily fast rates. The result also holds on the fixed thirty-nine-field menu at any fixed positive clock. No formula for that error floor is supplied, so the uncapped inverse-accuracy growth law remains open.

The earlier bounded-rate binary target class has three different prediction costs: exactly two states passively, $\Theta(\log(1/\delta))$ states for every constant-field step, and $\delta^{-\Theta(1)}$ states for arbitrary switching. This separation holds already for one-dimensional local walks with global refresh, where a polynomial switching predictor can preserve reversibility, the original field rule and spectral band. The constant-step upper has analytic field dependence but allows a different rate rule. Polynomial reversible switching cost also holds for typical labels on connected sparse expanders with a uniform local mixing gap. The binary reversibility separation uses a larger fixed rate budget than this earlier class. Its sharp-band refinement and a quantitative exponential lower for unbounded-rate reversible rivals remain open. Exponents remain unmatched. Originality is still under assessment, and manuscript writing remains on hold.
