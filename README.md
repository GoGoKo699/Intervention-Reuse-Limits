...
[Verification](docs/VERIFICATION.md) records the full suite and its limits. The certificates cover entire error boxes and explicit local realizations; they do not optimize the allowed rate error. Serial sampling and finite target preparation now have their own conditional guarantees and exposure budget. The next priority is operational justification or statistical certification of rival preparation and the joint-instrument and detector assumptions in a concrete device model. A target mixing bound cannot prepare arbitrary slow rivals, and visible stationarity does not certify hidden preparation. Further field or score-constant tuning is secondary. Manuscript drafting remains deferred.
