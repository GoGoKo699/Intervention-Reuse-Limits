...
These remain small absolute errors, not a one-percentage-point advantage or a practical sample-budget guarantee. The binary readout, stationary preparation, and control coupling only to the observed time-even conformation are substantive assumptions. The theorem concerns means; passive paths already contain memory, and full controlled path-law equality is not asserted.
...
[Verification](docs/VERIFICATION.md) records the full suite and its limits. The finite-margin verifier certifies the whole error box and one rational comparison model; it does not optimize over rival generators. Next priorities are physical and calibration robustness, the closest-source comparison, and the significance of the measured precision. Manuscript drafting remains deferred.
