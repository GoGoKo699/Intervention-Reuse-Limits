...
**Research status:** a fixed six-value actuator family now separates two reversible modeling architectures: every stationary-flux partition aggregate can require exponentially many states in a power of inverse accuracy, while a model with newly constructed states needs only polynomially many. Both preserve the original field rule and spectral band.

The earlier bounded-rate binary target class has three different prediction costs: exactly two states passively, $\Theta(\log(1/\delta))$ states for every constant-field step, and $\delta^{-\Theta(1)}$ states for arbitrary switching. This separation now holds already for one-dimensional local walks with global refresh, where a polynomial switching predictor can also preserve reversibility, the original field rule and spectral band. The constant-step upper has analytic field dependence but allows a different rate rule. The switching comparison now also holds for a typical-label class on connected sparse expanders whose local dynamics already has a uniform mixing gap; both earlier density and fragmentation conditions fail there. Switching remains hard at any fixed control clock; polynomial exponents are not matched. The spectral-cap-only reversible problem on general graphs remains open. Originality is still under assessment, and manuscript writing remains on hold.

## New result: merging existing states can have an exponential cost
